/**
 * exportPdf.ts — Shared PDF export engine for CMS and Offers modules.
 *
 * Uses html2canvas + jsPDF.
 * Resolves Tailwind v4 oklch() computed colors before capture so html2canvas
 * never encounters an unparseable color value.
 *
 * Public API:
 *   exportToPdf(elementId, options?)   — render → A4 PDF → browser download
 *   generatePdfBlob(elementId)         — render → A4 PDF → Blob (for upload/preview)
 *
 * Previous locations (now thin re-exports):
 *   src/modules/cms/utils/exportPdf.ts
 *   src/modules/offers/utils/exportPdf.ts
 */
...
// ── oklch → rgb resolver ──────────────────────────────────────────────────────
// Tailwind v4 CSS custom properties emit oklch(); html2canvas throws on them.
// Strategy: for every element in the clone, read computedStyle, resolve any
...
/** Render element → A4 PDF → return Blob (for upload / preview url). */
export async function generatePdfBlob(elementId: string): Promise<Blob> {
  ...
  // Clone into off-screen wrapper so we don't mutate the live DOM
  ...
  // Resolve oklch colors after element is in DOM
