...
export function useContracts() {
  ...
  const { asCmsClients, addClient: addSharedClient, updateClient: updateSharedClient, deleteClient: deleteSharedClient } = useSharedClients();
  ...
  // addWorkflowEvent — uses arrayUnion so concurrent writes never overwrite each other.
  const addWorkflowEvent = async (contractId: string, event: WorkflowEvent, newStatus?: string) => {
  ...
  const addClient = async (c: Omit<Client, 'id'>) => {
    await addSharedClient({ name_ar: c.name_ar, type: 'CUSTOMER', entity_type: c.entity_type as any, license_authority: c.license_authority, license_no: c.license_no, representative_name: c.representative_name, representative_title: c.representative_title, national_id: c.national_id, address: c.address, city: c.city, postal_code: c.postal_code, phone: c.phone, email: c.email, modules: ['cms'], status: 'active' });
  };
  ...
  const addProject = async (p: Omit<Project, 'id'>) => { const id = crypto.randomUUID(); await setDoc(doc(db, 'cms_projects', id), { ...p, id }); };
  ...
  return {
    contracts, clients: asCmsClients as unknown as Client[], templates, projects, loading,
    addContract, updateContract, deleteContract, addWorkflowEvent,
