...
export default function ContractEditor({
  ...
  // FIX: When clients load (Firestore is async), if the contract has no client_id
  // yet (was created before clients arrived), auto-set it to the first client.
  // This ensures the preview can always find a matching client object.
  useEffect(() => {
  ...
  const handleSelectTemplate = (template: ContractTemplate | null) => {
    const newContract: Contract = {
      ...
      // Use first available client id; will be updated via useEffect if clients arrive later
      ...
      if (contractId) {
        ...
        if (originalContract) {
          ...
          if (JSON.stringify(currentWithoutVersions) !== JSON.stringify(originalWithoutVersions)) {
            ...
            const newVersion: ContractVersion = { version_number: lastVersion + 1, created_at: new Date().toISOString(), change_summary: summary, snapshot: JSON.parse(JSON.stringify({ ...contractWithoutVersions, versions: [] })) as Omit<Contract, 'versions'> };
        ...
        const { versions: _v, ...contractWithoutVersions } = contract;
        const newVersion: ContractVersion = { version_number: 1, created_at: new Date().toISOString(), change_summary: '\u0625\u0646\u0634\u0627\u0621 \u0627\u0644\u0639\u0642\u062f', snapshot: JSON.parse(JSON.stringify({ ...contractWithoutVersions, versions: [] })) as Omit<Contract, 'versions'> };
  ...
  const confirmSaveAsTemplate = async () => {
    ...
    const newTemplate: ContractTemplate = { id: Date.now().toString(), name_ar: saveAsTemplateName.trim(), category: '\u0645\u062e\u0635\u0635', default_status: '\u0645\u0633\u0648\u062f\u0629', default_type: contract.type, default_articles: JSON.parse(JSON.stringify(contract.articles)), default_payment_schedule: JSON.parse(JSON.stringify(contract.payment_schedule)), default_appendices: JSON.parse(JSON.stringify(contract.appendices)), tags: contract.tags ? [...contract.tags] : [], is_default: false };
    ...
    <div className="flex flex-col h-full bg-slate-50">
      <header className="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between shrink-0 no-print">
        ...
        <div className="flex items-center space-x-3 space-x-reverse">
          {saveAsTemplateFeedback === 'success' && (<span className="text-sm text-emerald-700 bg-emerald-50 border border-emerald-200 px-3 py-1.5 rounded-lg flex items-center gap-1.5"><CheckCircle size={14} /> \u062a\u0645 \u062d\u0641\u0638 \u0627\u0644\u0642\u0627\u0644\u0628</span>)}
          <button onClick={handleSaveAsTemplate} className="flex items-center space-x-2 space-x-reverse bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 px-4 py-2 rounded-lg font-medium transition-colors"><FileCode2 size={18} /><span>{t('\u062d\u0641\u0638 \u0643\u0642\u0627\u0644\u0628', 'Save as Template', lang)}</span></button>
          <button onClick={handleSave} disabled={saving} className="flex items-center space-x-2 space-x-reverse bg-emerald-600 hover:bg-emerald-700 disabled:opacity-60 text-white px-4 py-2 rounded-lg font-medium transition-colors"><Save size={18} /><span>{saving ? t('\u062c\u0627\u0631\u064d...', 'Saving...', lang) : t('\u062d\u0641\u0638 \u0627\u0644\u0639\u0642\u062f', 'Save Contract', lang)}</span></button>
        </div>
      </header>
      ...
      {saveAsTemplateModal && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center no-print" dir="rtl">
          <div className="bg-white rounded-xl shadow-xl border border-slate-200 p-6 w-full max-w-md mx-4">
            ...
            <input type="text" dir="rtl" value={saveAsTemplateName} onChange={e => setSaveAsTemplateName(e.target.value)} onKeyDown={e => e.key === 'Enter' && confirmSaveAsTemplate()} autoFocus className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none mb-5" placeholder="\u0645\u062b\u0627\u0644: \u0642\u0627\u0644\u0628 \u062a\u0637\u0648\u064a\u0631 \u0645\u0646\u0635\u0629 SaaS" />
            <div className="flex gap-3 justify-end">
              ...
              <button onClick={confirmSaveAsTemplate} disabled={!saveAsTemplateName.trim()} className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white px-4 py-2 rounded-lg text-sm font-medium"><FileCode2 size={16} /> \u062d\u0641\u0638 \u0627\u0644\u0642\u0627\u0644\u0628</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="p-4 space-y-1">
            {tabs.map(tab => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id as Tab)}
                className={`w-full flex items-center space-x-3 space-x-reverse px-4 py-3 rounded-lg text-sm font-medium transition-colors ${activeTab === tab.id ? 'bg-emerald-50 text-emerald-700' : 'text-slate-600 hover:bg-slate-50'}`}>
    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 space-y-8">
      <div>
        <div className="flex justify-between items-center border-b border-slate-100 pb-4 mb-6">
          ...
          {selectedTemplate && (<div className="flex items-center space-x-2 space-x-reverse bg-blue-50 text-blue-700 px-3 py-1.5 rounded-lg text-sm font-medium border border-blue-100"><FileCode2 size={16} /><span>\u0642\u0627\u0644\u0628: {selectedTemplate.name_ar}</span></div>)}
        </div>
        <div className="grid grid-cols-2 gap-6">
          ...
          <div className="space-y-2">
            {/* FIX: Added placeholder option so that when client_id='', the select
                shows a visible hint instead of silently selecting nothing.
                The useEffect in ContractEditor auto-sets client_id when clients arrive. */}
            ...
            {!contract.client_id && clients?.length > 0 && (
              <p className="text-xs text-amber-600">\u064a\u0631\u062c\u0649 \u062a\u062d\u062f\u064a\u062f \u0627\u0644\u0639\u0645\u064a\u0644 \u0644\u0638\u0647\u0648\u0631\u0647 \u0641\u064a \u0627\u0644\u0645\u0639\u0627\u064a\u0646\u0629</p>
            )}
          ...
          <div>
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-5 text-sm text-slate-700" dir="rtl">
              <div className="flex items-start gap-3 mb-4">
                ...
                <div><p className="text-base font-bold text-slate-800">{sel.name_ar}</p><p className="text-xs text-slate-500 mt-0.5">\u0633.\u062a: {sel.cr_number}</p></div>
              </div>
...
export function ArticlesEditor({ contract, setContract, lang }: any) {
  ...
  const moveArticle = (index: number, dir: 'up' | 'down') => {
    if (dir === 'up' && index === 0) return; if (dir === 'down' && index === contract.articles.length - 1) return;
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button onClick={addArticle} className="flex items-center space-x-2 space-x-reverse text-sm font-medium text-emerald-600 bg-emerald-50 px-3 py-1.5 rounded-lg"><Plus size={16} /><span>{t('\u0625\u0636\u0627\u0641\u0629 \u0628\u0646\u062f', 'Add Article', lang)}</span></button>
      </div>
        ...
        {contract.articles.map((article: Article, index: number) => (
          <div key={article.id} className={`bg-white rounded-xl shadow-sm border ${article.is_visible ? 'border-slate-200' : 'border-dashed border-slate-300 opacity-50'} overflow-hidden`}>
            <div className="bg-slate-50 px-4 py-3 border-b border-slate-200 flex justify-between items-center">
              <div className="flex items-center space-x-3 space-x-reverse">
                <div className="flex flex-col space-y-1 ml-2"><button onClick={() => moveArticle(index, 'up')} className="text-slate-400 hover:text-emerald-600"><ArrowUp size={14} /></button><button onClick={() => moveArticle(index, 'down')} className="text-slate-400 hover:text-emerald-600"><ArrowDown size={14} /></button></div>
                ...
                <select value={article.article_type} onChange={e => updateArticle(article.id, 'article_type', e.target.value)} disabled={article.is_locked} className="text-xs bg-white border border-slate-200 rounded px-2 py-1 outline-none disabled:opacity-70">{['\u062a\u0645\u0647\u064a\u062f','\u0645\u0648\u0636\u0648\u0639','\u0645\u062f\u0629 \u0627\u0644\u062a\u0646\u0641\u064a\u0630','\u0627\u0644\u0642\u064a\u0645\u0629 \u0648\u0627\u0644\u062f\u0641\u0639\u0627\u062a','\u0627\u0644\u0645\u0644\u0643\u064a\u0629 \u0627\u0644\u0641\u0643\u0631\u064a\u0629','\u0625\u062f\u0627\u0631\u0629 \u0627\u0644\u0645\u0634\u0631\u0648\u0639','\u0637\u0644\u0628\u0627\u062a \u0627\u0644\u062a\u063a\u064a\u064a\u0631','\u0625\u0646\u0647\u0627\u0621 \u0627\u0644\u0627\u062a\u0641\u0627\u0642\u064a\u0629','\u0623\u062d\u0643\u0627\u0645 \u0639\u0627\u0645\u0629','\u0646\u0633\u062e \u0627\u0644\u0627\u062a\u0641\u0627\u0642\u064a\u0629','\u0645\u062e\u0635\u0635'].map(v => <option key={v} value={v}>{v}</option>)}</select>
              </div>
            ...
            <div className="p-4 relative">
              {article.blocks ? (
                <div className="space-y-4">
                    ...
                    <div key={block.id} className="relative group bg-white border border-slate-200 rounded-lg p-3">
                      ...
                      {block.type === 'list' && (
                        <div className="space-y-3">
                          <div className="flex items-center space-x-2 space-x-reverse mb-2"><span className="text-xs font-medium text-slate-500">\u0646\u0645\u0637 \u0627\u0644\u0642\u0627\u0626\u0645\u0629:</span><select value={block.style} onChange={e => updateBlock(article.id, block.id, { style: e.target.value as any })} disabled={article.is_locked} className="text-xs border border-slate-200 rounded px-2 py-1 outline-none"><option value="unordered">\u0646\u0642\u0637\u064a\u0629</option><option value="ordered">\u0631\u0642\u0645\u064a\u0629</option><option value="alpha">\u0623\u0628\u062c\u062f\u064a\u0629</option></select></div>
                  ...
                  {!article.is_locked && (<div className="flex items-center space-x-2 space-x-reverse pt-2 border-t border-slate-100"><button onClick={() => addBlock(article.id, 'paragraph')} className="flex items-center space-x-1 space-x-reverse px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-md text-xs font-medium"><AlignRight size={14} /> <span>\u0625\u0636\u0627\u0641\u0629 \u0641\u0642\u0631\u0629</span></button><button onClick={() => addBlock(article.id, 'list')} className="flex items-center space-x-1 space-x-reverse px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-md text-xs font-medium"><ListOrdered size={14} /> <span>\u0625\u0636\u0627\u0641\u0629 \u0642\u0627\u0626\u0645\u0629</span></button><button onClick={() => addBlock(article.id, 'page_break')} className="flex items-center space-x-1 space-x-reverse px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-md text-xs font-medium"><FileMinus size={14} /> <span>\u0641\u0627\u0635\u0644 \u0635\u0641\u062d\u0629</span></button></div>)}
                </div>
              ) : (
                <div className="space-y-3">
              ...
              {article.is_locked && (<div className="absolute inset-0 flex items-center justify-center pointer-events-none z-20"><div className="bg-white/90 px-4 py-2 rounded-lg flex items-center space-x-2 space-x-reverse text-slate-500 shadow-sm border border-slate-100"><Lock size={16} /><span className="text-sm font-medium">\u0647\u0630\u0627 \u0627\u0644\u0628\u0646\u062f \u0645\u062d\u0645\u064a</span></div></div>)}
            </div>
          </div>
        ))}
      </div>
...
export function PaymentsEditor({ contract, setContract, lang }: any) {
  ...
  const taskLogic = (tasks: TaskRow[], insts: Installment[]) => { const sub = tasks.reduce((s: number, t: TaskRow) => s + Number(t.cost_sar || 0), 0); const vat = sub * (schedule.vat_rate / 100); const total = sub + vat; upd({ tasks, subtotal_sar: sub, vat_amount: vat, total_sar: total, installments: insts.map((i: Installment) => ({ ...i, amount_sar: total * (i.percentage / 100), amount_words_ar: numberToArabicWords(total * (i.percentage / 100)) })) }); };
  ...
  return (
    <div className="space-y-8">
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50"><h3 className="font-bold text-slate-800">{t('\u0627\u0644\u0645\u0647\u0627\u0645 \u0648\u0627\u0644\u062a\u0643\u0627\u0644\u064a\u0641','Tasks & Costs',lang)}</h3><button onClick={addTask} className="text-sm font-medium text-emerald-600 flex items-center space-x-1 space-x-reverse"><Plus size={16} /> <span>{t('\u0625\u0636\u0627\u0641\u0629 \u0645\u0647\u0645\u0629','Add Task',lang)}</span></button></div>
        <div className="overflow-x-auto"><table className="w-full text-right text-sm">
          <thead className="bg-slate-50 text-slate-500 border-b border-slate-200"><tr><th className="px-6 py-3 font-medium">{t('\u0627\u0644\u0645\u0647\u0645\u0629','Task',lang)}</th><th className="px-6 py-3 font-medium">{t('\u0627\u0644\u0645\u062f\u0629','Duration',lang)}</th><th className="px-6 py-3 font-medium">{t('\u0627\u0644\u062a\u0643\u0644\u0641\u0629 (\u0631.\u0633)','Cost (SAR)',lang)}</th><th className="px-6 py-3 font-medium">{t('\u0627\u0644\u062a\u0643\u0631\u0627\u0631','Frequency',lang)}</th><th className="px-6 py-3 w-16"></th></tr></thead>
          <tbody className="divide-y divide-slate-100">{schedule.tasks.map((task: TaskRow) => (<tr key={task.id}><td className="px-6 py-3"><input type="text" dir="rtl" value={task.task_name_ar} onChange={e => updTask(task.id,'task_name_ar',e.target.value)} className="w-full p-2 border border-slate-200 rounded focus:ring-1 focus:ring-emerald-500 outline-none" /></td><td className="px-6 py-3"><input type="text" dir="rtl" value={task.duration} onChange={e => updTask(task.id,'duration',e.target.value)} className="w-full p-2 border border-slate-200 rounded focus:ring-1 focus:ring-emerald-500 outline-none" /></td><td className="px-6 py-3"><input type="number" value={task.cost_sar} onChange={e => updTask(task.id,'cost_sar',Number(e.target.value))} className="w-full p-2 border border-slate-200 rounded focus:ring-1 focus:ring-emerald-500 outline-none" /></td><td className="px-6 py-3"><select value={task.frequency} onChange={e => updTask(task.id,'frequency',e.target.value)} className="w-full p-2 border border-slate-200 rounded focus:ring-1 focus:ring-emerald-500 outline-none"><option value="\u0645\u0631\u0629 \u0648\u0627\u062d\u062f\u0629">\u0645\u0631\u0629 \u0648\u0627\u062d\u062f\u0629</option><option value="\u0633\u0646\u0648\u064a">\u0633\u0646\u0648\u064a</option><option value="\u0634\u0647\u0631\u064a">\u0634\u0647\u0631\u064a</option></select></td><td className="px-6 py-3 text-center"><button onClick={() => rmTask(task.id)} className="text-slate-400 hover:text-red-500"><Trash2 size={16} /></button></td></tr>))}</tbody>
          <tfoot className="bg-slate-50 border-t border-slate-200"><tr><td colSpan={2} className="px-6 py-3 text-left font-medium text-slate-600">{t('\u0627\u0644\u0625\u062c\u0645\u0627\u0644\u064a','Subtotal',lang)}</td><td className="px-6 py-3 font-bold text-slate-800">{schedule.subtotal_sar.toLocaleString()}</td><td colSpan={2}></td></tr><tr><td colSpan={2} className="px-6 py-3 text-left font-medium text-slate-600">{t('\u0636\u0631\u064a\u0628\u0629 \u0627\u0644\u0642\u064a\u0645\u0629 \u0627\u0644\u0645\u0636\u0627\u0641\u0629 (15%)','VAT (15%)',lang)}</td><td className="px-6 py-3 font-bold text-slate-800">{schedule.vat_amount.toLocaleString()}</td><td colSpan={2}></td></tr><tr className="bg-emerald-50"><td colSpan={2} className="px-6 py-3 text-left font-bold text-emerald-800">{t('\u0627\u0644\u0625\u062c\u0645\u0627\u0644\u064a \u0627\u0644\u0643\u0644\u064a','Total',lang)}</td><td className="px-6 py-3 font-bold text-emerald-800">{schedule.total_sar.toLocaleString()}</td><td colSpan={2}></td></tr></tfoot>
        </table></div>
      </div>
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50"><div className="flex items-center space-x-4 space-x-reverse"><h3 className="font-bold text-slate-800">{t('\u062c\u062f\u0648\u0644 \u0627\u0644\u062f\u0641\u0639\u0627\u062a','Installments',lang)}</h3><span className={`text-xs font-bold px-2.5 py-1 rounded-full ${totalPct===100?'bg-emerald-100 text-emerald-700':'bg-red-100 text-red-700'}`}>{totalPct}%</span></div><button onClick={addInst} className="text-sm font-medium text-emerald-600 flex items-center space-x-1 space-x-reverse"><Plus size={16} /> <span>{t('\u0625\u0636\u0627\u0641\u0629 \u062f\u0641\u0639\u0629','Add Installment',lang)}</span></button></div>
        <div className="overflow-x-auto"><table className="w-full text-right text-sm">
          <thead className="bg-slate-50 text-slate-500 border-b border-slate-200"><tr><th className="px-6 py-3 font-medium">{t('\u0627\u0644\u062f\u0641\u0639\u0629','Label',lang)}</th><th className="px-6 py-3 font-medium">{t('\u0627\u0644\u0627\u0633\u062a\u062d\u0642\u0627\u0642','Trigger',lang)}</th><th className="px-6 py-3 font-medium">{t('\u0627\u0644\u0646\u0633\u0628\u0629 (%)','%',lang)}</th><th className="px-6 py-3 font-medium">{t('\u0627\u0644\u0645\u0628\u0644\u063a','Amount',lang)}</th><th className="px-6 py-3 w-16"></th></tr></thead>
          <tbody className="divide-y divide-slate-100">{schedule.installments.map((inst: Installment) => (<tr key={inst.id}><td className="px-6 py-3"><input type="text" dir="rtl" value={inst.label_ar} onChange={e => updInst(inst.id,'label_ar',e.target.value)} className="w-full p-2 border border-slate-200 rounded focus:ring-1 focus:ring-emerald-500 outline-none" /></td><td className="px-6 py-3"><select value={inst.trigger_event} onChange={e => updInst(inst.id,'trigger_event',e.target.value)} className="w-full p-2 border border-slate-200 rounded focus:ring-1 focus:ring-emerald-500 outline-none">{['\u062a\u0648\u0642\u064a\u0639 \u0627\u0644\u0639\u0642\u062f','\u0627\u0639\u062a\u0645\u0627\u062f \u0627\u0644\u0645\u062a\u0637\u0644\u0628\u0627\u062a','\u0627\u0644\u0625\u0637\u0644\u0627\u0642 \u0627\u0644\u062a\u062c\u0631\u064a\u0628\u064a','\u0627\u0644\u0625\u0637\u0644\u0627\u0642','\u0645\u062e\u0635\u0635'].map(v => <option key={v} value={v}>{v}</option>)}</select></td><td className="px-6 py-3"><input type="number" value={inst.percentage} onChange={e => updInst(inst.id,'percentage',Number(e.target.value))} className="w-full p-2 border border-slate-200 rounded focus:ring-1 focus:ring-emerald-500 outline-none" /></td><td className="px-6 py-3"><div className="font-medium text-slate-700">{inst.amount_sar.toLocaleString()} \u0631.\u0633</div><div className="text-xs text-slate-500 mt-1">{inst.amount_words_ar}</div></td><td className="px-6 py-3 text-center"><button onClick={() => rmInst(inst.id)} className="text-slate-400 hover:text-red-500"><Trash2 size={16} /></button></td></tr>))}</tbody>
        </table></div>
      </div>
...
export function AppendicesEditor({ contract, setContract, lang }: any) {
  ...
  const move = (index: number, dir: 'up' | 'down') => { if (dir==='up'&&index===0) return; if (dir==='down'&&index===contract.appendices.length-1) return; const arr=[...contract.appendices]; const ti=dir==='up'?index-1:index+1; [arr[index],arr[ti]]=[arr[ti],arr[index]]; setContract({...contract,appendices:arr.map((a,i)=>({...a,order_index:i+1}))}); };
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center"><h2 className="text-lg font-bold text-slate-800">{t('\u0645\u0644\u0627\u062d\u0642 \u0627\u0644\u0639\u0642\u062f','Appendices',lang)}</h2><button onClick={add} className="flex items-center space-x-2 space-x-reverse text-sm font-medium text-emerald-600 bg-emerald-50 px-3 py-1.5 rounded-lg"><Plus size={16} /><span>{t('\u0625\u0636\u0627\u0641\u0629 \u0645\u0644\u062d\u0642','Add Appendix',lang)}</span></button></div>
      <div className="space-y-4">
        {contract.appendices.map((app: Appendix, index: number) => (
          <div key={app.id} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="bg-slate-50 px-4 py-3 border-b border-slate-200 flex justify-between items-center">
              <div className="flex items-center space-x-3 space-x-reverse"><div className="flex flex-col space-y-1 ml-2"><button onClick={() => move(index,'up')} className="text-slate-400 hover:text-emerald-600"><ArrowUp size={14} /></button><button onClick={() => move(index,'down')} className="text-slate-400 hover:text-emerald-600"><ArrowDown size={14} /></button></div><span className="text-sm font-bold text-slate-500 w-16">\u0645\u0644\u062d\u0642 {index+1}</span><input type="text" dir="rtl" value={app.title_ar} onChange={e => upd(app.id,'title_ar',e.target.value)} className="bg-transparent border-none focus:ring-0 font-bold text-slate-800 w-48 px-0 outline-none" /><select value={app.appendix_type} onChange={e => upd(app.id,'appendix_type',e.target.value)} className="text-xs bg-white border border-slate-200 rounded px-2 py-1 outline-none">{['\u0642\u0627\u0626\u0645\u0629 \u0627\u0644\u062e\u062f\u0645\u0627\u062a','\u0627\u0644\u062a\u0647\u064a\u0626\u0629 \u0627\u0644\u062a\u0642\u0646\u064a\u0629','\u0627\u0644\u0639\u0631\u0636 \u0627\u0644\u0641\u0646\u064a','\u0642\u0627\u0626\u0645\u0629 \u0627\u0644\u0623\u0633\u0639\u0627\u0631','\u0623\u062e\u0631\u0649'].map(v => <option key={v} value={v}>{v}</option>)}</select></div>
...
function AttachmentsEditor({ contract, setContract, lang }: any) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center"><h2 className="text-lg font-bold text-slate-800">{t('\u0627\u0644\u0645\u0631\u0641\u0642\u0627\u062a','Attachments',lang)}</h2><button onClick={()=>setShowForm(!showForm)} className="flex items-center space-x-2 space-x-reverse text-sm font-medium text-emerald-600 bg-emerald-50 px-3 py-1.5 rounded-lg"><Plus size={16}/><span>{t('\u0625\u0636\u0627\u0641\u0629 \u0645\u0631\u0641\u0642','Add Attachment',lang)}</span></button></div>
      {showForm&&(<div className="bg-white rounded-xl shadow-sm border border-emerald-200 p-4 flex items-end gap-4"><div className="flex-1"><label className="text-sm font-medium text-slate-700">\u0627\u0633\u0645 \u0627\u0644\u0645\u0631\u0641\u0642</label><input type="text" dir="rtl" value={newTitle} onChange={e=>setNewTitle(e.target.value)} className="w-full mt-1 px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none" /></div><div className="w-48"><label className="text-sm font-medium text-slate-700">\u0646\u0648\u0639 \u0627\u0644\u0645\u0631\u0641\u0642</label><select value={newType} onChange={e=>setNewType(e.target.value as any)} className="w-full mt-1 px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none">{['\u0627\u0644\u0639\u0631\u0636 \u0627\u0644\u0641\u0646\u064a','\u0627\u0644\u062a\u0635\u0627\u0645\u064a\u0645','\u0648\u062b\u064a\u0642\u0629 \u0627\u0644\u0646\u0637\u0627\u0642','\u0623\u062e\u0631\u0649'].map(v=><option key={v} value={v}>{v}</option>)}</select></div><div className="flex gap-2"><button onClick={add} className="bg-emerald-600 text-white px-4 py-2 rounded-lg font-medium">\u0625\u0636\u0627\u0641\u0629</button><button onClick={()=>setShowForm(false)} className="bg-slate-100 text-slate-700 px-4 py-2 rounded-lg font-medium">\u0625\u0644\u063a\u0627\u0621</button></div></div>)}
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden"><table className="w-full text-right text-sm"><thead className="bg-slate-50 text-slate-500 border-b border-slate-200"><tr><th className="px-6 py-3 font-medium">{t('\u0627\u0633\u0645 \u0627\u0644\u0645\u0631\u0641\u0642','Title',lang)}</th><th className="px-6 py-3 font-medium">{t('\u0627\u0644\u0646\u0648\u0639','Type',lang)}</th><th className="px-6 py-3 font-medium">{t('\u062a\u0627\u0631\u064a\u062e \u0627\u0644\u0631\u0641\u0639','Uploaded At',lang)}</th><th className="px-6 py-3 w-16"></th></tr></thead><tbody className="divide-y divide-slate-100">{contract.attachments.map((a:Attachment)=>(<tr key={a.id}><td className="px-6 py-4 font-medium text-slate-800">{a.title}</td><td className="px-6 py-4 text-slate-600">{a.attachment_type}</td><td className="px-6 py-4 text-slate-500">{a.uploaded_at}</td><td className="px-6 py-4 text-center"><button onClick={()=>del(a.id)} className="text-slate-400 hover:text-red-500"><Trash2 size={16}/></button></td></tr>))}{contract.attachments.length===0&&<tr><td colSpan={4} className="px-6 py-8 text-center text-slate-500">{t('\u0644\u0627 \u062a\u0648\u062c\u062f \u0645\u0631\u0641\u0642\u0627\u062a','No attachments',lang)}</td></tr>}</tbody></table></div>
    </div>
  );
}

function VersionsEditor({ contract, setContract, lang }: any) {
  const restore = (v: ContractVersion) => { if (confirm('\u0647\u0644 \u0623\u0646\u062a \u0645\u062a\u0623\u0643\u062f \u0645\u0646 \u0627\u0633\u062a\u0639\u0627\u062f\u0629 \u0647\u0630\u0627 \u0627\u0644\u0625\u0635\u062f\u0627\u0631\u061f')) setContract({...v.snapshot,versions:contract.versions}); };
  return (
    <div className="space-y-6">
      <h2 className="text-lg font-bold text-slate-800">{t('\u0633\u062c\u0644 \u0627\u0644\u0625\u0635\u062f\u0627\u0631\u0627\u062a','Version History',lang)}</h2>
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden"><table className="w-full text-right text-sm"><thead className="bg-slate-50 text-slate-500 border-b border-slate-200"><tr><th className="px-6 py-3 font-medium">{t('\u0631\u0642\u0645 \u0627\u0644\u0625\u0635\u062f\u0627\u0631','Version',lang)}</th><th className="px-6 py-3 font-medium">{t('\u0627\u0644\u062a\u0627\u0631\u064a\u062e','Date',lang)}</th><th className="px-6 py-3 font-medium">{t('\u0645\u0644\u062e\u0635 \u0627\u0644\u062a\u063a\u064a\u064a\u0631','Summary',lang)}</th><th className="px-6 py-3 w-24"></th></tr></thead><tbody className="divide-y divide-slate-100">{contract.versions.map((v:ContractVersion)=>(<tr key={v.version_number}><td className="px-6 py-4 font-bold text-slate-800">v{v.version_number}.0</td><td className="px-6 py-4 text-slate-500" dir="ltr">{new Date(v.created_at).toLocaleString('ar-SA')}</td><td className="px-6 py-4 text-slate-700">{v.change_summary}</td><td className="px-6 py-4 text-center"><button onClick={()=>restore(v)} className="text-emerald-600 hover:text-emerald-700 font-medium">{t('\u0627\u0633\u062a\u0639\u0627\u062f\u0629','Restore',lang)}</button></td></tr>))}{contract.versions.length===0&&<tr><td colSpan={4} className="px-6 py-8 text-center text-slate-500">{t('\u0633\u064a\u062a\u0645 \u0625\u0646\u0634\u0627\u0621 \u0627\u0644\u0625\u0635\u062f\u0627\u0631\u0627\u062a \u062a\u0644\u0642\u0627\u0626\u064a\u0627\u064b \u0639\u0646\u062f \u062d\u0641\u0638 \u0627\u0644\u0639\u0642\u062f','Versions are created automatically when saving',lang)}</td></tr>}</tbody></table></div>
    </div>
  );
}
...
function ContractPreview({ contract, lang, projects, clients }: any) {
  ...
  const handleDrive = async () => {
    ...
    try { if (!isConnected()) await requestDriveAccess(); const blob = await generatePdfBlob('contract-preview'); const r = await uploadPdfToDrive(blob, san(`${contract.contract_number}-${contract.title_ar}.pdf`), entity.name_ar, client?.name_ar); setDriveLink(r.webViewLink); setDriveStatus('success'); }
  ...
  return (
    <div>
      <div className="mb-4 flex justify-end gap-3 no-print sticky top-0 z-10 bg-slate-50 py-2">
        <button onClick={handleDrive} disabled={driveStatus==='uploading'} className="flex items-center space-x-2 space-x-reverse bg-blue-600 hover:bg-blue-700 disabled:opacity-60 text-white px-6 py-2 rounded-lg font-medium"><Cloud size={18}/><span>{driveStatus==='uploading'?t('\u062c\u0627\u0631\u064d \u0627\u0644\u0631\u0641\u0639...','Uploading...',lang):t('\u062d\u0641\u0638 \u0641\u064a Drive','Save to Drive',lang)}</span></button>
        <button onClick={handlePdf} disabled={isExporting} className="flex items-center space-x-2 space-x-reverse bg-red-600 hover:bg-red-700 disabled:opacity-60 text-white px-6 py-2 rounded-lg font-medium"><FileDown size={18}/><span>{isExporting?t('\u062c\u0627\u0631\u064d \u0627\u0644\u062a\u0635\u062f\u064a\u0631...','Exporting...',lang):t('\u062a\u0635\u062f\u064a\u0631 PDF','Export PDF',lang)}</span></button>
        <button onClick={() => window.print()} className="flex items-center space-x-2 space-x-reverse bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-2 rounded-lg font-medium"><Printer size={18}/><span>{t('\u0637\u0628\u0627\u0639\u0629','Print',lang)}</span></button>
      </div>
      ...
      <div id="contract-preview" className="bg-white shadow-xl border border-slate-200 p-12 max-w-4xl mx-auto text-slate-900 contract-pdf-ready" dir="rtl" style={{fontFamily:"'Tajawal', sans-serif"}}>
        <div className="flex justify-between items-start mb-8"><div>{entity.logo_base64?<img src={entity.logo_base64} alt="logo" className="h-20 max-w-[180px] object-contain" />:<div className="h-20 w-[180px]" />}</div><div className="text-left text-sm text-slate-500 leading-6"><p>{entity.name_ar}</p><p>\u0633.\u062a: {entity.cr_number}</p><p>{entity.city}</p></div></div>
        <div className="text-center mb-8"><h1 className="text-2xl font-bold mb-6">\u0628\u0633\u0645 \u0627\u0644\u0644\u0647 \u0627\u0644\u0631\u062d\u0645\u0646 \u0627\u0644\u0631\u062d\u064a\u0645</h1><h2 className="text-xl font-bold">{contract.title_ar}</h2><p className="text-sm text-slate-500 mt-2">\u0631\u0642\u0645 \u0627\u0644\u0639\u0642\u062f: {contract.contract_number}</p><p className="text-sm text-slate-500">\u0627\u0644\u0645\u064a\u0644\u0627\u062f\u064a: {contract.start_date} | \u0627\u0644\u0647\u062c\u0631\u064a: {toHijri(contract.start_date)}</p></div>
        <div className="mb-10 leading-relaxed text-lg">
          ...
          <div className="mb-6 bg-slate-50 p-4 rounded border border-slate-100">
            ...
            <div className="grid grid-cols-2 gap-2 text-base"><p>\u0633\u062c\u0644 \u062a\u062c\u0627\u0631\u064a \u0631\u0642\u0645: {entity.cr_number}</p><p>\u0648\u064a\u0645\u062b\u0644\u0647\u0627: {entity.representative_name} ({entity.representative_title})</p><p>\u0627\u0644\u0639\u0646\u0648\u0627\u0646: {entity.city} \u2013 {entity.address}</p><p>\u0627\u0644\u0628\u0631\u064a\u062f: {entity.email}</p></div>
          </div>
          <div className="mb-6 bg-slate-50 p-4 rounded border border-slate-100">
            {/* Party Two — client must be non-null for this block to be meaningful */}
            {client ? (
              <>
              ...
              <p className="text-amber-600 font-medium">\u26a0\ufe0f \u0644\u0645 \u064a\u062a\u0645 \u062a\u062d\u062f\u064a\u062f \u0627\u0644\u0639\u0645\u064a\u0644 \u2014 \u064a\u0631\u062c\u0649 \u0627\u062e\u062a\u064a\u0627\u0631\u0647 \u0641\u064a \u062a\u0628\u0648\u064a\u0628 \u0645\u0639\u0644\u0648\u0645\u0627\u062a \u0627\u0644\u0639\u0642\u062f</p>
            )}
          </div>
        </div>
            ...
            return (
              <div key={article.id} className="article-block">
                ...
                {article.blocks&&article.blocks.length>0?(
                  <div className="space-y-4">{article.blocks.map((block:ArticleBlock)=>{
                ...
                {article.article_type==='\u0627\u0644\u0642\u064a\u0645\u0629 \u0648\u0627\u0644\u062f\u0641\u0639\u0627\u062a'&&schedule.tasks.length>0&&(
                  <div className="mt-6">
                    <table className="w-full border-collapse border border-slate-300 mb-6 text-base">
                      ...
                      <tfoot style={{backgroundColor:'#f8fafc',fontWeight:'bold'}}><tr><td colSpan={2} className="border border-slate-300 px-4 py-2">\u0627\u0644\u0625\u062c\u0645\u0627\u0644\u064a \u063a\u064a\u0631 \u0634\u0627\u0645\u0644 \u0627\u0644\u0636\u0631\u064a\u0628\u0629</td><td className="border border-slate-300 px-4 py-2">{schedule.subtotal_sar.toLocaleString()}</td></tr><tr><td colSpan={2} className="border border-slate-300 px-4 py-2">\u0636\u0631\u064a\u0628\u0629 \u0627\u0644\u0642\u064a\u0645\u0629 \u0627\u0644\u0645\u0636\u0627\u0641\u0629 ({schedule.vat_rate}%)</td><td className="border border-slate-300 px-4 py-2">{schedule.vat_amount.toLocaleString()}</td></tr><tr style={{backgroundColor:colors.secondary}}><td colSpan={2} className="border border-slate-300 px-4 py-2" style={{color:colors.accent}}>\u0627\u0644\u0625\u062c\u0645\u0627\u0644\u064a \u0627\u0644\u0643\u0644\u064a</td><td className="border border-slate-300 px-4 py-2" style={{color:colors.accent}}>{schedule.total_sar.toLocaleString()}</td></tr></tfoot>
                    </table>
                    ...
                    <div className="p-4 border border-slate-200 rounded text-base" style={{backgroundColor:colors.secondary}}><p className="font-bold mb-2">\u0627\u0644\u0628\u064a\u0627\u0646\u0627\u062a \u0627\u0644\u0628\u0646\u0643\u064a\u0629:</p><p>\u0627\u0633\u0645 \u0627\u0644\u0628\u0646\u0643: {schedule.bank_name}</p><p>\u0627\u0633\u0645 \u0627\u0644\u062d\u0633\u0627\u0628: {schedule.account_holder}</p><p>\u0631\u0642\u0645 \u0627\u0644\u0622\u064a\u0628\u0627\u0646: <span dir="ltr">{schedule.bank_iban}</span></p></div>
                  </div>
                )}
              </div>
            );
