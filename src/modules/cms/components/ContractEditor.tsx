  ...
  const handleSelectTemplate = (template: ContractTemplate | null) => {
    const newContract: Contract = {
      ...
      payment_schedule: template && template.default_payment_schedule ? JSON.parse(JSON.stringify(template.default_payment_schedule)) : {
        subtotal_sar: 0, vat_rate: 15, vat_amount: 0, total_sar: 0,
        bank_iban: 'SA865134841770007', bank_name: 'بنك البلاد',
        account_holder: 'شركة دراية الذكية لتقنية المعلومات', tasks: [], installments: []
      },
        ...
        <div className="max-w-4xl mx-auto w-full space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            {templates.map((template: ContractTemplate) => (
              <button key={template.id} onClick={() => handleSelectTemplate(template)} className="flex flex-col items-start p-6 bg-white border border-slate-200 rounded-xl hover:border-emerald-500 hover:shadow-md transition-all text-right">
                <div className="flex justify-between w-full mb-4">
                  ...
                  <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-slate-100 text-slate-800 h-6">{template.category}</span>
                </div>
      ...
      <header className="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between shrink-0 no-print">
        ...
        <div className="flex items-center space-x-3 space-x-reverse">
          ...
          <button onClick={handleSave} className="flex items-center space-x-2 space-x-reverse bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg font-medium transition-colors">
            <Save size={18} /><span>{t('حفظ العقد', 'Save Contract', lang)}</span>
          </button>
        </div>
      </header>
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <div className="space-y-2">
            ...
            <select value={contract.project_id || ''} onChange={e => {
              ...
              {projects?.map((project: any) => <option key={project.id} value={project.id}>{project.name_ar}</option>)}
            </select>
            {selectedProject && (
          ...
          <div className="space-y-2">
            ...
            <select value={contract.entity_id || getDefaultEntity().id} onChange={e => setContract({ ...contract, entity_id: e.target.value })} className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none">
              {settings.entities.map(entity => <option key={entity.id} value={entity.id}>{entity.name_ar} {entity.is_default ? '(الافتراضي)' : ''}</option>)}
            </select>
            {(() => {
            ...
            <select value={contract.status} onChange={e => {
              ...
              if (newStatus === 'موقّع') {
                platformBus.emit(PLATFORM_EVENTS.CONTRACT_SIGNED, {
                  contractId: contract.id, contractTitle: contract.title_ar,
                  counterpartyId: contract.client_id, projectId: contract.project_id,
                  amount: contract.payment_schedule.total_sar, currency: 'SAR',
                });
              }
            }} className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none">
          ...
          <div className="space-y-2">
            ...
            <select value={contract.client_id} onChange={e => setContract({...contract, client_id: e.target.value})} className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none">
              {(clients || []).map((client: any) => <option key={client.id} value={client.id}>{client.name_ar}</option>)}
            </select>
            {showClientWarning && <p className="text-xs text-amber-600 mt-1">سيتم تحديث العميل تلقائياً ليتطابق مع المشروع المختار</p>}
          </div>
          <div className="space-y-2">
          ...
          <div className="space-y-2 col-span-2">
            ...
            <input type="text" dir="rtl" value={contract.tags?.join('، ') || ''} onChange={e => setContract({...contract, tags: e.target.value.split('،').map((t: string) => t.trim()).filter(Boolean)})} className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none" />
          </div>
        </div>
      </div>
      {(() => {
        ...
        return (
          <div>
            <div className="flex items-center justify-between border-b border-slate-100 pb-4 mb-6">
              ...
              <span className="text-xs text-slate-400 bg-slate-100 px-3 py-1 rounded-full">{t('تُقرأ من إعدادات الجهة المختارة أعلاه', 'Read from selected entity settings', lang)}</span>
            </div>
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-5 text-sm text-slate-700" dir="rtl">
              <div className="flex items-start gap-3 mb-4">
                {selectedEntity.logo_base64 && <img src={selectedEntity.logo_base64} alt="logo" className="h-14 w-28 object-contain border border-slate-200 rounded-lg p-1 bg-white shrink-0" />}
                <div>
                  <p className="text-base font-bold text-slate-800">{selectedEntity.name_ar}</p>
                  <p className="text-xs text-slate-500 mt-0.5">المرخصة من قبل وزارة التجارة والصناعة برقم {selectedEntity.cr_number}</p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm border-t border-slate-200 pt-4">
                <div className="flex gap-2"><span className="text-slate-500 shrink-0">الممثل:</span><span className="font-medium">{selectedEntity.representative_name} — {selectedEntity.representative_title}</span></div>
                <div className="flex gap-2"><span className="text-slate-500 shrink-0">العنوان:</span><span>{selectedEntity.city} — {selectedEntity.address}</span></div>
                <div className="flex gap-2"><span className="text-slate-500 shrink-0">الهاتف:</span><span dir="ltr">{selectedEntity.phone}</span></div>
                <div className="flex gap-2"><span className="text-slate-500 shrink-0">البريد الإلكتروني:</span><span dir="ltr">{selectedEntity.email}</span></div>
                <div className="flex gap-2"><span className="text-slate-500 shrink-0">البنك:</span><span>{selectedEntity.bank_name} — {selectedEntity.account_holder}</span></div>
                <div className="flex gap-2 col-span-2"><span className="text-slate-500 shrink-0">الآيبان:</span><span dir="ltr" className="font-mono">{selectedEntity.bank_iban}</span></div>
              </div>
            </div>
          </div>
        );
...
export function ArticlesEditor({ contract, setContract, lang }: any) {
  const addArticle = () => {
    const newArticle: Article = { id: Date.now().toString(), order_index: contract.articles.length + 1, title_ar: 'بند جديد', body_ar: '', article_type: 'مخصص', is_locked: false, is_visible: true };
  ...
  const moveArticle = (index: number, direction: 'up' | 'down') => {
    ...
    setContract({ ...contract, articles: newArticles.map((a, idx) => ({ ...a, order_index: idx + 1 })) });
  };
  const addBlock = (articleId: string, type: 'paragraph' | 'list' | 'page_break') => {
    ...
    if (type === 'paragraph') newBlock = { id: blockId, type: 'paragraph', text_ar: '' };
    else if (type === 'list') newBlock = { id: blockId, type: 'list', style: 'unordered', items: [{ id: blockId + '1', text_ar: '' }] };
    else newBlock = { id: blockId, type: 'page_break' };
    updateArticle(articleId, 'blocks', [...(article.blocks || []), newBlock]);
  };
  const updateBlock = (articleId: string, blockId: string, updates: Partial<ArticleBlock>) => {
    const article = contract.articles.find((a: Article) => a.id === articleId);
    if (!article || !article.blocks) return;
    updateArticle(articleId, 'blocks', article.blocks.map((b: ArticleBlock) => b.id === blockId ? { ...b, ...updates } : b));
  };
  ...
  const convertToBlocks = (articleId: string) => {
    const article = contract.articles.find((a: Article) => a.id === articleId);
    if (!article) return;
    updateArticle(articleId, 'blocks', [{ id: Date.now().toString(), type: 'paragraph', text_ar: article.body_ar || '' } as ParagraphBlock]);
  };
          ...
          <div key={article.id} className={`bg-white rounded-xl shadow-sm border ${article.is_visible ? 'border-slate-200' : 'border-dashed border-slate-300 opacity-50'} overflow-hidden`}>
            <div className="bg-slate-50 px-4 py-3 border-b border-slate-200 flex justify-between items-center">
              <div className="flex items-center space-x-3 space-x-reverse">
                ...
                <input type="text" dir="rtl" value={article.title_ar} onChange={e => updateArticle(article.id, 'title_ar', e.target.value)} disabled={article.is_locked} className="bg-transparent border-none focus:ring-0 font-bold text-slate-800 w-48 px-0 outline-none disabled:opacity-70" placeholder="عنوان البند" />
                ...
                {!article.is_visible && <span className="bg-slate-200 text-slate-600 text-xs px-2 py-1 rounded font-medium">مخفي في المعاينة</span>}
              </div>
            ...
            <div className="p-4 relative">
              {article.blocks ? (
                          ...
                          <div className="space-y-2">
                            {block.items.map((item, iIndex) => (
                              <div key={item.id} className="flex items-start space-x-2 space-x-reverse">
                                ...
                                <input type="text" dir="rtl" value={item.text_ar} onChange={e => { const newItems = [...block.items]; newItems[iIndex].text_ar = e.target.value; updateBlock(article.id, block.id, { items: newItems }); }} disabled={article.is_locked} className="flex-1 p-1.5 border-b border-transparent hover:border-slate-200 focus:border-emerald-500 bg-transparent outline-none transition-colors" placeholder={`عنصر ${iIndex + 1}`} />
                                {!article.is_locked && <button onClick={() => { const newItems = block.items.filter((_, idx) => idx !== iIndex); updateBlock(article.id, block.id, { items: newItems }); }} className="p-1.5 text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity"><X size={14} /></button>}
                              </div>
                            ))}
                          </div>
                          {!article.is_locked && <button onClick={() => { const newItems = [...block.items, { id: Date.now().toString(), text_ar: '' }]; updateBlock(article.id, block.id, { items: newItems }); }} className="text-xs text-emerald-600 hover:text-emerald-700 font-medium flex items-center space-x-1 space-x-reverse mt-2"><Plus size={12} /> <span>إضافة عنصر</span></button>}
                        </div>
                      )}
                      {block.type === 'page_break' && <div className="flex flex-col items-center justify-center py-6 border-2 border-dashed border-slate-200 rounded-lg bg-slate-50 text-slate-400"><FileMinus size={24} className="mb-2 opacity-50" /><span className="text-sm font-medium">فاصل صفحة قبل المحتوى التالي</span></div>}
                    </div>
                  ))}
                  {!article.is_locked && (
                    <div className="flex items-center space-x-2 space-x-reverse pt-2 border-t border-slate-100">
                      ...
                      <button onClick={() => addBlock(article.id, 'page_break')} className="flex items-center space-x-1 space-x-reverse px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-md text-xs font-medium"><FileMinus size={14} /> <span>إضافة فاصل صفحة</span></button>
                    </div>
                  )}
                </div>
              ) : (
                <div className="space-y-3">
                  ...
                  {!article.is_locked && <button onClick={() => convertToBlocks(article.id)} className="text-xs font-medium text-emerald-600 hover:text-emerald-700 bg-emerald-50 px-3 py-1.5 rounded-md">الترقية إلى كتل المحتوى (Blocks)</button>}
                </div>
              )}
              {article.is_locked && (
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-20">
                  <div className="bg-white/90 px-4 py-2 rounded-lg flex items-center space-x-2 space-x-reverse text-slate-500 shadow-sm border border-slate-100"><Lock size={16} /><span className="text-sm font-medium">هذا البند محمي ولا يمكن تعديله</span></div>
                </div>
              )}
            </div>
          </div>
...
export function PaymentsEditor({ contract, setContract, lang }: any) {
  ...
  const updateSchedule = (updates: Partial<PaymentSchedule>) => setContract({ ...contract, payment_schedule: { ...schedule, ...updates } });
  const updateTaskLogic = (newTasks: TaskRow[], currentInstallments: Installment[]) => {
    ...
    updateSchedule({ tasks: newTasks, subtotal_sar: subtotal, vat_amount: vat, total_sar: total, installments: currentInstallments.map((inst: Installment) => { const amt = total * (inst.percentage / 100); return { ...inst, amount_sar: amt, amount_words_ar: numberToArabicWords(amt) }; }) });
  };
  const addTask = () => updateTaskLogic([...schedule.tasks, { id: Date.now().toString(), task_name_ar: 'مهمة جديدة', duration: '', cost_sar: 0, frequency: 'مرة واحدة' }], schedule.installments);
  const updateTask = (id: string, field: keyof TaskRow, value: any) => updateTaskLogic(schedule.tasks.map((t: TaskRow) => t.id === id ? { ...t, [field]: value } : t), schedule.installments);
  const removeTask = (id: string) => updateTaskLogic(schedule.tasks.filter((t: TaskRow) => t.id !== id), schedule.installments);
  const addInstallment = () => updateSchedule({ installments: [...schedule.installments, { id: Date.now().toString(), label_ar: 'دفعة جديدة', trigger_event: 'توقيع العقد', percentage: 0, amount_sar: 0, amount_words_ar: 'صفر ريال سعودي' }] });
  const updateInstallment = (id: string, field: keyof Installment, value: any) => updateSchedule({ installments: schedule.installments.map((i: Installment) => { if (i.id !== id) return i; const updated = { ...i, [field]: value }; if (field === 'percentage') { const amt = schedule.total_sar * (Number(value) / 100); updated.amount_sar = amt; updated.amount_words_ar = numberToArabicWords(amt); } return updated; }) });
  const removeInstallment = (id: string) => updateSchedule({ installments: schedule.installments.filter((i: Installment) => i.id !== id) });
    ...
    <div className="space-y-8">
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
          ...
          <button onClick={addTask} className="text-sm font-medium text-emerald-600 hover:text-emerald-700 flex items-center space-x-1 space-x-reverse"><Plus size={16} /> <span>{t('إضافة مهمة', 'Add Task', lang)}</span></button>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-right text-sm">
            <thead className="bg-slate-50 text-slate-500 border-b border-slate-200"><tr>
              <th className="px-6 py-3 font-medium">{t('المهمة', 'Task', lang)}</th><th className="px-6 py-3 font-medium">{t('المدة', 'Duration', lang)}</th><th className="px-6 py-3 font-medium">{t('التكلفة (ر.س)', 'Cost (SAR)', lang)}</th><th className="px-6 py-3 font-medium">{t('التكرار', 'Frequency', lang)}</th><th className="px-6 py-3 font-medium w-16"></th>
            </tr></thead>
            <tbody className="divide-y divide-slate-100">
              {schedule.tasks.map((task: TaskRow) => (
                <tr key={task.id}>
                  <td className="px-6 py-3"><input type="text" dir="rtl" value={task.task_name_ar} onChange={e => updateTask(task.id, 'task_name_ar', e.target.value)} className="w-full p-2 border border-slate-200 rounded focus:ring-1 focus:ring-emerald-500 outline-none" /></td>
                  <td className="px-6 py-3"><input type="text" dir="rtl" value={task.duration} onChange={e => updateTask(task.id, 'duration', e.target.value)} className="w-full p-2 border border-slate-200 rounded focus:ring-1 focus:ring-emerald-500 outline-none" /></td>
                  <td className="px-6 py-3"><input type="number" value={task.cost_sar} onChange={e => updateTask(task.id, 'cost_sar', Number(e.target.value))} className="w-full p-2 border border-slate-200 rounded focus:ring-1 focus:ring-emerald-500 outline-none" /></td>
                  <td className="px-6 py-3"><select value={task.frequency} onChange={e => updateTask(task.id, 'frequency', e.target.value)} className="w-full p-2 border border-slate-200 rounded focus:ring-1 focus:ring-emerald-500 outline-none"><option value="مرة واحدة">مرة واحدة</option><option value="سنوي">سنوي</option><option value="شهري">شهري</option></select></td>
                  <td className="px-6 py-3 text-center"><button onClick={() => removeTask(task.id)} className="text-slate-400 hover:text-red-500"><Trash2 size={16} /></button></td>
                </tr>
              ))}
            </tbody>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
          <div className="flex items-center space-x-4 space-x-reverse">
            ...
            <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${totalPercentage === 100 ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>{totalPercentage}%</span>
          </div>
          <button onClick={addInstallment} className="text-sm font-medium text-emerald-600 hover:text-emerald-700 flex items-center space-x-1 space-x-reverse"><Plus size={16} /> <span>{t('إضافة دفعة', 'Add Installment', lang)}</span></button>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-right text-sm">
            <thead className="bg-slate-50 text-slate-500 border-b border-slate-200"><tr>
              <th className="px-6 py-3 font-medium">{t('الدفعة', 'Label', lang)}</th><th className="px-6 py-3 font-medium">{t('الاستحقاق', 'Trigger', lang)}</th><th className="px-6 py-3 font-medium">{t('النسبة (%)', 'Percentage', lang)}</th><th className="px-6 py-3 font-medium">{t('المبلغ', 'Amount', lang)}</th><th className="px-6 py-3 font-medium w-16"></th>
            </tr></thead>
            <tbody className="divide-y divide-slate-100">
              {schedule.installments.map((inst: Installment) => (
                <tr key={inst.id}>
                  <td className="px-6 py-3"><input type="text" dir="rtl" value={inst.label_ar} onChange={e => updateInstallment(inst.id, 'label_ar', e.target.value)} className="w-full p-2 border border-slate-200 rounded focus:ring-1 focus:ring-emerald-500 outline-none" /></td>
                  <td className="px-6 py-3"><select value={inst.trigger_event} onChange={e => updateInstallment(inst.id, 'trigger_event', e.target.value)} className="w-full p-2 border border-slate-200 rounded focus:ring-1 focus:ring-emerald-500 outline-none"><option value="توقيع العقد">توقيع العقد</option><option value="اعتماد المتطلبات">اعتماد المتطلبات</option><option value="الإطلاق التجريبي">الإطلاق التجريبي</option><option value="الإطلاق">الإطلاق</option><option value="مخصص">مخصص</option></select></td>
                  <td className="px-6 py-3"><input type="number" value={inst.percentage} onChange={e => updateInstallment(inst.id, 'percentage', Number(e.target.value))} className="w-full p-2 border border-slate-200 rounded focus:ring-1 focus:ring-emerald-500 outline-none" /></td>
                  ...
                  <td className="px-6 py-3 text-center"><button onClick={() => removeInstallment(inst.id)} className="text-slate-400 hover:text-red-500"><Trash2 size={16} /></button></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="space-y-2"><label className="text-sm font-medium text-slate-700">{t('اسم البنك', 'Bank Name', lang)}</label><input type="text" value={schedule.bank_name} onChange={e => updateSchedule({bank_name: e.target.value})} className="w-full px-3 py-2 border border-slate-300 rounded-lg outline-none focus:ring-2 focus:ring-emerald-500" /></div>
          <div className="space-y-2"><label className="text-sm font-medium text-slate-700">{t('اسم الحساب', 'Account Holder', lang)}</label><input type="text" value={schedule.account_holder} onChange={e => updateSchedule({account_holder: e.target.value})} className="w-full px-3 py-2 border border-slate-300 rounded-lg outline-none focus:ring-2 focus:ring-emerald-500" /></div>
          <div className="space-y-2"><label className="text-sm font-medium text-slate-700">{t('رقم الآيبان', 'IBAN', lang)}</label><input type="text" dir="ltr" value={schedule.bank_iban} onChange={e => updateSchedule({bank_iban: e.target.value})} className="w-full px-3 py-2 border border-slate-300 rounded-lg outline-none focus:ring-2 focus:ring-emerald-500 text-left" /></div>
        </div>
      </div>
    </div>
  );
...
export function AppendicesEditor({ contract, setContract, lang }: any) {
  const addAppendix = () => setContract({ ...contract, appendices: [...contract.appendices, { id: Date.now().toString(), order_index: contract.appendices.length + 1, title_ar: 'ملحق جديد', body_ar: '', appendix_type: 'أخرى' } as Appendix] });
  const updateAppendix = (id: string, field: keyof Appendix, value: any) => setContract({ ...contract, appendices: contract.appendices.map((a: Appendix) => a.id === id ? { ...a, [field]: value } : a) });
  const deleteAppendix = (id: string) => setContract({ ...contract, appendices: contract.appendices.filter((a: Appendix) => a.id !== id).map((a: Appendix, idx: number) => ({ ...a, order_index: idx + 1 })) });
  const moveAppendix = (index: number, direction: 'up' | 'down') => {
    ...
    setContract({ ...contract, appendices: newApps.map((a, idx) => ({ ...a, order_index: idx + 1 })) });
  };
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-lg font-bold text-slate-800">{t('ملاحق العقد', 'Appendices', lang)}</h2>
        <button onClick={addAppendix} className="flex items-center space-x-2 space-x-reverse text-sm font-medium text-emerald-600 hover:text-emerald-700 bg-emerald-50 px-3 py-1.5 rounded-lg"><Plus size={16} /><span>{t('إضافة ملحق', 'Add Appendix', lang)}</span></button>
      </div>
      <div className="space-y-4">
        {contract.appendices.map((app: Appendix, index: number) => (
          <div key={app.id} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="bg-slate-50 px-4 py-3 border-b border-slate-200 flex justify-between items-center">
              <div className="flex items-center space-x-3 space-x-reverse">
                <div className="flex flex-col space-y-1 ml-2"><button onClick={() => moveAppendix(index, 'up')} className="text-slate-400 hover:text-emerald-600"><ArrowUp size={14} /></button><button onClick={() => moveAppendix(index, 'down')} className="text-slate-400 hover:text-emerald-600"><ArrowDown size={14} /></button></div>
                <span className="text-sm font-bold text-slate-500 w-16">ملحق {index + 1}</span>
                <input type="text" dir="rtl" value={app.title_ar} onChange={e => updateAppendix(app.id, 'title_ar', e.target.value)} className="bg-transparent border-none focus:ring-0 font-bold text-slate-800 w-48 px-0 outline-none" placeholder="عنوان الملحق" />
                <select value={app.appendix_type} onChange={e => updateAppendix(app.id, 'appendix_type', e.target.value)} className="text-xs bg-white border border-slate-200 rounded px-2 py-1 outline-none">
                  {['قائمة الخدمات','التهيئة التقنية','العرض الفني','قائمة الأسعار','أخرى'].map(v => <option key={v} value={v}>{v}</option>)}
                </select>
              </div>
              <button onClick={() => deleteAppendix(app.id)} className="text-slate-400 hover:text-red-500 p-1.5"><Trash2 size={16} /></button>
            </div>
            <div className="p-4"><textarea dir="rtl" value={app.body_ar} onChange={e => updateAppendix(app.id, 'body_ar', e.target.value)} className="w-full h-32 p-3 border border-slate-200 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none resize-y" placeholder="محتوى الملحق..." /></div>
          </div>
        ))}
...
function AttachmentsEditor({ contract, setContract, lang }: any) {
  ...
  const addAttachment = () => { if (!newTitle.trim()) return; setContract({ ...contract, attachments: [...contract.attachments, { id: Date.now().toString(), title: newTitle.trim(), file_type: 'PDF', attachment_type: newType, uploaded_at: new Date().toISOString().split('T')[0] } as Attachment] }); setNewTitle(''); setNewType('أخرى'); setShowForm(false); };
  const deleteAttachment = (id: string) => setContract({ ...contract, attachments: contract.attachments.filter((a: Attachment) => a.id !== id) });
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-lg font-bold text-slate-800">{t('المرفقات', 'Attachments', lang)}</h2>
        <button onClick={() => setShowForm(!showForm)} className="flex items-center space-x-2 space-x-reverse text-sm font-medium text-emerald-600 hover:text-emerald-700 bg-emerald-50 px-3 py-1.5 rounded-lg"><Plus size={16} /><span>{t('إضافة مرفق', 'Add Attachment', lang)}</span></button>
      </div>
      {showForm && (
        <div className="bg-white rounded-xl shadow-sm border border-emerald-200 p-4 flex items-end gap-4">
          <div className="flex-1 space-y-2"><label className="text-sm font-medium text-slate-700">اسم المرفق</label><input type="text" dir="rtl" value={newTitle} onChange={e => setNewTitle(e.target.value)} placeholder="مثال: العرض الفني المعتمد" className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none" /></div>
          <div className="w-48 space-y-2"><label className="text-sm font-medium text-slate-700">نوع المرفق</label><select value={newType} onChange={e => setNewType(e.target.value as any)} className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none">{['العرض الفني','التصاميم','وثيقة النطاق','أخرى'].map(v => <option key={v} value={v}>{v}</option>)}</select></div>
          <div className="flex gap-2"><button onClick={addAttachment} className="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg font-medium">إضافة</button><button onClick={() => setShowForm(false)} className="bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-2 rounded-lg font-medium">إلغاء</button></div>
        </div>
      )}
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <table className="w-full text-right text-sm">
          <thead className="bg-slate-50 text-slate-500 border-b border-slate-200"><tr><th className="px-6 py-3 font-medium">{t('اسم المرفق', 'Title', lang)}</th><th className="px-6 py-3 font-medium">{t('النوع', 'Type', lang)}</th><th className="px-6 py-3 font-medium">{t('تاريخ الرفع', 'Uploaded At', lang)}</th><th className="px-6 py-3 font-medium w-16"></th></tr></thead>
          <tbody className="divide-y divide-slate-100">
            {contract.attachments.map((att: Attachment) => (<tr key={att.id}><td className="px-6 py-4 font-medium text-slate-800">{att.title}</td><td className="px-6 py-4 text-slate-600">{att.attachment_type}</td><td className="px-6 py-4 text-slate-500">{att.uploaded_at}</td><td className="px-6 py-4 text-center"><button onClick={() => deleteAttachment(att.id)} className="text-slate-400 hover:text-red-500"><Trash2 size={16} /></button></td></tr>))}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <table className="w-full text-right text-sm">
          <thead className="bg-slate-50 text-slate-500 border-b border-slate-200"><tr>
            <th className="px-6 py-3 font-medium">{t('رقم الإصدار', 'Version', lang)}</th><th className="px-6 py-3 font-medium">{t('التاريخ', 'Date', lang)}</th><th className="px-6 py-3 font-medium">{t('ملخص التغيير', 'Summary', lang)}</th><th className="px-6 py-3 font-medium w-24"></th>
          </tr></thead>
          <tbody className="divide-y divide-slate-100">
            {contract.versions.map((v: ContractVersion) => (
              <tr key={v.version_number}>
                ...
                <td className="px-6 py-4 text-center"><button onClick={() => restoreVersion(v)} className="text-emerald-600 hover:text-emerald-700 font-medium">{t('استعادة', 'Restore', lang)}</button></td>
              </tr>
            ))}
            {contract.versions.length === 0 && <tr><td colSpan={4} className="px-6 py-8 text-center text-slate-500">{t('سيتم إنشاء الإصدارات تلقائياً عند حفظ العقد', 'Versions are created automatically when saving the contract', lang)}</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
...
function ContractPreview({ contract, lang, projects, clients }: any) {
  ...
  const handleSaveToDrive = async () => {
    ...
    try {
      ...
      const result = await uploadPdfToDrive(blob, filename, entity.name_ar, client?.name_ar);
      setDriveLink(result.webViewLink); setDriveStatus('success');
    } catch (err) { console.error(err); setDriveStatus('error'); }
  };
  const handleExportPdf = async () => {
    setIsExporting(true);
    try { await exportContractToPdf('contract-preview', { filename: sanitizeFilename(`${contract.contract_number}-${contract.title_ar}.pdf`), primaryColor: colors.primary }); }
    catch (err) { console.error('PDF export failed:', err); setExportError(true); setTimeout(() => setExportError(false), 4000); }
    finally { setIsExporting(false); }
  };
  const visibleCount = visibleArticles.length;
  const arabicNumbers: Record<number, string> = { 1: 'بند واحد', 2: 'بندين', 3: 'ثلاثة بنود', 4: 'أربعة بنود', 5: 'خمسة بنود', 6: 'ستة بنود', 7: 'سبعة بنود', 8: 'ثمانية بنود', 9: 'تسعة بنود', 10: 'عشرة بنود', 11: 'أحد عشر بنداً', 12: 'اثني عشر بنداً', 13: 'ثلاثة عشر بنداً', 14: 'أربعة عشر بنداً', 15: 'خمسة عشر بنداً' };
  ...
  return (
    <div>
      <div className="mb-4 flex justify-end gap-3 no-print sticky top-0 z-10 bg-slate-50 py-2">
        <button onClick={handleSaveToDrive} disabled={driveStatus === 'uploading'} className="flex items-center space-x-2 space-x-reverse bg-blue-600 hover:bg-blue-700 disabled:opacity-60 text-white px-6 py-2 rounded-lg font-medium">
          <Cloud size={18} /><span>{driveStatus === 'uploading' ? t('جارٍ الرفع...', 'Uploading...', lang) : t('حفظ في Drive', 'Save to Drive', lang)}</span>
        </button>
        <button onClick={handleExportPdf} disabled={isExporting} className="flex items-center space-x-2 space-x-reverse bg-red-600 hover:bg-red-700 disabled:opacity-60 text-white px-6 py-2 rounded-lg font-medium">
      ...
      {driveStatus === 'success' && driveLink && (<div className="mb-4 flex items-center gap-2 text-sm text-blue-700 bg-blue-50 border border-blue-200 rounded-lg px-4 py-2 no-print"><CheckCircle size={16} /><span>{t('تم الحفظ في Google Drive', 'Saved to Google Drive', lang)}</span><a href={driveLink} target="_blank" rel="noopener noreferrer" className="mr-auto underline font-medium">{t('فتح الملف', 'Open file', lang)}</a></div>)}
      {driveStatus === 'error' && (<div className="mb-4 flex items-center gap-2 text-sm text-red-700 bg-red-50 border border-red-200 rounded-lg px-4 py-2 no-print"><AlertCircle size={16} /><span>{t('فشل الرفع، حاول مرة أخرى', 'Upload failed, try again', lang)}</span></div>)}
      {exportError && (<div className="mb-4 flex items-center gap-2 text-sm text-red-700 bg-red-50 border border-red-200 rounded-lg px-4 py-2 no-print"><AlertCircle size={16} /><span>{t('فشل تصدير PDF، حاول مرة أخرى', 'PDF export failed, please try again', lang)}</span></div>)}
      <div id="contract-preview" className="bg-white shadow-xl border border-slate-200 p-12 max-w-4xl mx-auto text-slate-900 contract-pdf-ready" dir="rtl" style={{ fontFamily: "'Tajawal', sans-serif" }}>
        <div className="flex justify-between items-start mb-8">
          <div className="text-right">{entity.logo_base64 ? <img src={entity.logo_base64} alt="logo" className="h-20 max-w-[180px] object-contain" /> : <div className="h-20 w-[180px]" />}</div>
        ...
        <div className="text-center mb-8">
          ...
          {project && (<div className="mt-4 inline-block bg-slate-50 border border-slate-200 rounded-lg px-6 py-3 text-sm text-slate-600"><div className="flex items-center justify-center gap-6"><div><span className="text-slate-400 ml-1">المشروع:</span><span className="font-medium">{project.name_ar}</span></div><div><span className="text-slate-400 ml-1">نوع المشروع:</span><span className="font-medium">{project.project_type}</span></div><div><span className="text-slate-400 ml-1">القيمة:</span><span className="font-medium">{project.amount_sar.toLocaleString('ar-SA')} ر.س</span></div></div></div>)}
        </div>
                  ...
                  <div className="space-y-4">
                    {article.blocks.map((block: ArticleBlock) => {
                      if (block.type === 'paragraph') {
                        ...
                        return (<ListTag key={block.id} className={`${listClass} list-inside space-y-2 mr-4 leading-relaxed text-justify`}>{block.items.map(item => <li key={item.id}>{item.text_ar}</li>)}</ListTag>);
