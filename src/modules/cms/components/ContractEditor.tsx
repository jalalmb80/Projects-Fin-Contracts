...
import { Contract, Article, ArticleBlock, ParagraphBlock, TaskRow, Installment, PaymentSchedule, Appendix, Attachment, ContractVersion, ContractTemplate, WorkflowEvent } from '../types';
import { Save, X, Plus, Trash2, Eye, FileText, Users, CreditCard, Paperclip, History, Lock, Unlock, ArrowUp, ArrowDown, Printer, AlignRight, ListOrdered, FileMinus, FileCode2, FileDown, Cloud, CheckCircle, AlertCircle, GitBranch } from 'lucide-react';
...
import WorkflowNoteModal from './WorkflowNoteModal';
import WorkflowTimeline from './WorkflowTimeline';
import { useContracts } from '../hooks/useContracts';

type Tab = 'metadata' | 'articles' | 'payments' | 'appendices' | 'attachments' | 'versions' | 'workflow' | 'preview';
...
export default function ContractEditor({
  ...
  const { addWorkflowEvent } = useContracts();
  ...
  // Finance project creation prompt (shown when status → win status)
  const [showFinanceModal, setShowFinanceModal] = useState(false);
  const [contractForModal, setContractForModal] = useState<Contract | null>(null);

  // Workflow modals
  const [pendingStatus, setPendingStatus] = useState<string | null>(null);  // status awaiting workflow modal
  ...
  // ── Workflow: status change intercepted from MetadataEditor ─────────────────
  // MetadataEditor calls this instead of directly setting status.
  // We capture the pending status and open the transition modal.
  ...
  // Called when WorkflowTransitionModal confirms
  const handleTransitionConfirm = async (event: WorkflowEvent) => {
    if (!contract || pendingStatus === null) return;
    const newStatus = pendingStatus;

    // 1. Persist: status + workflow event in one write
    if (contractId) {
      await addWorkflowEvent(contractId, event, newStatus);
    }

    // 2. Update local state so the editor reflects the change immediately
    setContract(prev => prev ? {
      ...prev,
      status: newStatus,
      workflow_events: [event, ...(prev.workflow_events ?? [])],
    } : prev);

    // 3. Win-status side-effects (Finance modal + bus event)
    ...
    if (isNowWin && !wasWin) {
      setContractForModal({ ...contract, status: newStatus });
  ...
  // Called when WorkflowNoteModal confirms
  const handleNoteConfirm = async (event: WorkflowEvent) => {
    if (!contract) return;
    if (contractId) {
      await addWorkflowEvent(contractId, event);
    }
    setContract(prev => prev ? {
      ...prev,
      workflow_events: [event, ...(prev.workflow_events ?? [])],
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (contractId) {
        ...
        if (orig) {
          // Compare content only (exclude workflow_events and versions from diff)
          const cur = { ...contract, versions: [], workflow_events: [] };
          const old = { ...orig, versions: [], workflow_events: [] };
  ...
  const workflowEventCount = contract.workflow_events?.length ?? 0;
  ...
  const tabs = [
    ...
    { id: 'workflow',    label_ar: 'سجل الإجراءات',  label_en: 'Workflow',       icon: GitBranch, badge: workflowEventCount },
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50">
      ...
      {showTransitionModal && contract && pendingStatus && (
        <WorkflowTransitionModal
          contractId={contract.id}
          fromStatus={contract.status}
          toStatus={pendingStatus}
          onConfirm={handleTransitionConfirm}
          onCancel={() => { setShowTransitionModal(false); setPendingStatus(null); }}
        />
      )}

      {/* Workflow note modal */}
      {showNoteModal && contract && (
        <WorkflowNoteModal
          ...
          <div className="p-4 space-y-1">
            {tabs.map(tab => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id as Tab)}
                ...
                <span className="flex-1 text-right">{t(tab.label_ar, tab.label_en, lang)}</span>
          ...
          <div className="max-w-4xl mx-auto">
            ...
            {activeTab === 'workflow'    && (
              <WorkflowTimeline
                ...
                onStatusChange={() => {
                  // Open transition modal with a "pick new status" flow:
                  // We set pendingStatus to empty string as sentinel; the modal
                  // is re-used but fromStatus/toStatus will be set once user picks.
                  // Simpler: navigate to metadata tab where the status select lives.
...
// ─────────────────────────────────────────────────────────────────────────────
// MetadataEditor
// Status select now calls onStatusChangeRequest (opens workflow modal)
// instead of directly mutating contract.status.
// ─────────────────────────────────────────────────────────────────────────────
function MetadataEditor({ contract, setContract, lang, projects, clients, templates, onStatusChangeRequest }: any) {
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <div className="space-y-2">
            ...
            <select value={contract.type} onChange={e => setContract({...contract, type: e.target.value})} className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none">
              {contractTypes.map((type: string) => (<option key={type} value={type}>{type}</option>))}
            </select>
          </div>
          <div className="space-y-2">
          ...
          {/* Status — now routes through workflow modal */}
          <div className="space-y-2">
            <label className="text-sm font-medium text-slate-700 flex items-center gap-2">
              {t('حالة العقد', 'Status', lang)}
              <span className="text-xs text-amber-600 bg-amber-50 border border-amber-200 px-2 py-0.5 rounded-full">
                {t('يطلب تسجيل', 'workflow tracked', lang)}
              </span>
            </label>
            <select
              value={contract.status}
              onChange={e => onStatusChangeRequest(e.target.value)}
              className="w-full px-4 py-2 border border-amber-300 rounded-lg focus:ring-2 focus:ring-amber-400 outline-none bg-amber-50/30"
            >
            ...
            <p className="text-xs text-slate-400">
              {t('تغيير الحالة يفتح نافذة تسجيل المسؤول والملاحظة', 'Changing status opens a workflow log dialog', lang)}
            </p>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-right text-sm">
            ...
            <tfoot className="bg-slate-50 border-t border-slate-200">
              <tr><td colSpan={2} className="px-6 py-3 text-left font-medium text-slate-600">{t('الإجمالي','الإجمالي',lang)}</td><td className="px-6 py-3 font-bold text-slate-800">{schedule.subtotal_sar.toLocaleString()}</td><td colSpan={2}></td></tr>
          ...
          <div key={app.id} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="bg-slate-50 px-4 py-3 border-b border-slate-200 flex justify-between items-center">
              <div className="flex items-center space-x-3 space-x-reverse">
                ...
                <select value={app.appendix_type} onChange={e => upd(app.id,'appendix_type',e.target.value)} className="text-xs bg-white border border-slate-200 rounded px-2 py-1 outline-none">{appendixTypes.map(v => <option key={v} value={v}>{v}</option>)}</select>
              </div>
  ...
  return (
    <div>
      ...
      <div id="contract-preview" className="bg-white shadow-xl border border-slate-200 p-12 max-w-4xl mx-auto text-slate-900 contract-pdf-ready" dir="rtl" style={{fontFamily:"'Tajawal', sans-serif"}}>
        <div className="flex justify-between items-start mb-8"><div>{entity.logo_base64 ? <img src={entity.logo_base64} alt="logo" className="h-20 max-w-[180px] object-contain" /> : <div className="h-20 w-[180px]" />}</div><div className="text-left text-sm text-slate-500 leading-6"><p>{entity.name_ar}</p><p>{t('س.ت','CR',lang)}: {entity.cr_number}</p><p>{entity.city}</p></div></div>
        <div className="text-center mb-8"><h1 className="text-2xl font-bold mb-6">بسم الله الرحمن الرحيم</h1><h2 className="text-xl font-bold">{contract.title_ar}</h2><p className="text-sm text-slate-500 mt-2">{t('رقم العقد','Contract No.',lang)}: {contract.contract_number}</p><p className="text-sm text-slate-500">{t('الميلادي','Date',lang)}: {contract.start_date} | {t('الهجري','Hijri',lang)}: {toHijri(contract.start_date)}</p></div>
        <div className="mb-10 leading-relaxed text-lg">
          ...
          <div className="mb-6 bg-slate-50 p-4 rounded border border-slate-100"><p className="font-bold mb-2" style={{color:colors.primary}}>{t('الطرف الأول','Party One',lang)}: {entity.name_ar}</p><div className="grid grid-cols-2 gap-2 text-base"><p>{t('سجل تجاري رقم','CR No.',lang)}: {entity.cr_number}</p><p>{t('ويمثلها','Represented by',lang)}: {entity.representative_name} ({entity.representative_title})</p><p>{t('العنوان','Address',lang)}: {entity.city} – {entity.address}</p><p>{t('البريد','Email',lang)}: {entity.email}</p></div></div>
          <div className="mb-6 bg-slate-50 p-4 rounded border border-slate-100">{client ? (<><p className="font-bold mb-3" style={{color:colors.primary}}>{t('الطرف الثاني','Party Two',lang)}: {client.name_ar}</p><div className="grid grid-cols-2 gap-2 text-base"><p>{t('السجل التجاري','CR',lang)}: {client.license_no||'—'}</p><p>{t('ويمثلها','Rep.',lang)}: {client.representative_name||'—'} — {client.representative_title||'—'}</p><p>{t('المدينة','City',lang)}: {client.city||'—'}</p><p>{t('العنوان','Address',lang)}: {client.address||'—'}</p>{client.phone&&<p>{t('الهاتف','Phone',lang)}: <span dir="ltr">{client.phone}</span></p>}{client.email&&<p>{t('البريد','Email',lang)}: <span dir="ltr">{client.email}</span></p>}</div></>) : (<p className="text-amber-600 font-medium">⚠️ {t('لم يتم تحديد العميل — يرجى اختياره في تبويب معلومات العقد', 'No client selected — please choose one in the Metadata tab', lang)}</p>)}</div>
        </div>
            ...
            return (
              <div key={article.id} className="article-block">
                ...
                {article.article_type==='القيمة والدفعات'&&schedule.tasks.length>0&&(
                  <div className="mt-6">
                    <table className="w-full border-collapse border border-slate-300 mb-6 text-base"><thead style={{backgroundColor:colors.primary}}><tr><th className="border border-slate-300 px-4 py-2 text-right text-white font-medium">{t('المهمة','Task',lang)}</th><th className="border border-slate-300 px-4 py-2 text-right text-white font-medium">{t('المدة','Duration',lang)}</th><th className="border border-slate-300 px-4 py-2 text-right text-white font-medium">{t('التكلفة (ريال)','Cost (SAR)',lang)}</th></tr></thead><tbody>{schedule.tasks.map((task:TaskRow,i:number)=>(<tr key={task.id} style={{backgroundColor:i%2===0?colors.secondary:'#fff'}}><td className="border border-slate-300 px-4 py-2">{task.task_name_ar}</td><td className="border border-slate-300 px-4 py-2">{task.duration}</td><td className="border border-slate-300 px-4 py-2">{task.cost_sar.toLocaleString()}</td></tr>))}</tbody><tfoot style={{backgroundColor:'#f8fafc',fontWeight:'bold'}}><tr><td colSpan={2} className="border border-slate-300 px-4 py-2">{t('الإجمالي غير شامل الضريبة','Subtotal excl. VAT',lang)}</td><td className="border border-slate-300 px-4 py-2">{schedule.subtotal_sar.toLocaleString()}</td></tr><tr><td colSpan={2} className="border border-slate-300 px-4 py-2">{t('ضريبة القيمة المضافة','VAT',lang)} ({schedule.vat_rate}%)</td><td className="border border-slate-300 px-4 py-2">{schedule.vat_amount.toLocaleString()}</td></tr><tr style={{backgroundColor:colors.secondary}}><td colSpan={2} className="border border-slate-300 px-4 py-2" style={{color:colors.accent}}>{t('الإجمالي الكلي','Total',lang)}</td><td className="border border-slate-300 px-4 py-2" style={{color:colors.accent}}>{schedule.total_sar.toLocaleString()}</td></tr></tfoot></table>
                    {schedule.installments.length>0&&(<div className="mb-6"><p className="font-bold mb-2">{t('طريقة الدفع','Payment schedule',lang)}:</p><ul className="list-disc list-inside space-y-4 mr-4">{schedule.installments.map((inst:Installment)=>(<li key={inst.id}><span className="font-bold">{inst.label_ar}</span>: {inst.percentage}% {t('تستحق عند','due at',lang)} {inst.trigger_event}.<div className="mt-1 text-slate-700">{t('المبلغ','Amount',lang)}: {inst.amount_sar.toLocaleString()} {t('ريال','SAR',lang)} ({inst.amount_words_ar})</div></li>))}</ul></div>)}
                    <div className="p-4 border border-slate-200 rounded text-base" style={{backgroundColor:colors.secondary}}><p className="font-bold mb-2">{t('البيانات البنكية','Bank Details',lang)}:</p><p>{t('اسم البنك','Bank',lang)}: {schedule.bank_name}</p><p>{t('اسم الحساب','Account',lang)}: {schedule.account_holder}</p><p>{t('رقم الآيبان','IBAN',lang)}: <span dir="ltr">{schedule.bank_iban}</span></p></div>
                  </div>
                )}
              </div>
            );
          })}
          <div className="mt-24 grid grid-cols-2 gap-12 text-center text-lg signature-block"><div><p className="font-bold mb-8">{t('الطرف الأول','Party One',lang)}</p><p>{t('الاسم','Name',lang)}: {entity.representative_name}</p><p className="mt-4">{t('التوقيع','Signature',lang)}: ___________________</p></div><div><p className="font-bold mb-8">{t('الطرف الثاني','Party Two',lang)}</p><p>{t('الاسم','Name',lang)}: {client?.representative_name||'—'}</p><p className="mt-4">{t('التوقيع','Signature',lang)}: ___________________</p></div></div>
          {contract.appendices.map((app:Appendix,idx:number)=>(<div key={app.id} className="contract-page-break" style={{pageBreakBefore:'always',marginTop:'48px',paddingTop:'32px',borderTop:'2px solid #1e293b'}}>{idx===0&&<h2 className="text-2xl font-bold mb-8 text-center">{t('الملاحق','Appendices',lang)}</h2>}<h3 className="text-xl font-bold mb-3" style={{color:colors.accent}}>{t('الملحق','Appendix',lang)} {idx+1}: {app.title_ar}</h3><div className="whitespace-pre-wrap leading-relaxed text-justify">{app.body_ar||t('(نص الملحق فارغ)','(empty)',lang)}</div></div>))}
          {contract.attachments.length>0&&(<div className="contract-page-break" style={{pageBreakBefore:'always',marginTop:'48px',paddingTop:'32px',borderTop:'2px solid #1e293b'}}><h2 className="text-2xl font-bold mb-8 text-center">{t('المرفقات','Attachments',lang)}</h2><table className="w-full border-collapse border border-slate-300 text-base"><thead style={{backgroundColor:colors.primary}}><tr><th className="border border-slate-300 px-4 py-2 text-right text-white font-medium">{t('اسم المرفق','Title',lang)}</th><th className="border border-slate-300 px-4 py-2 text-right text-white font-medium">{t('النوع','Type',lang)}</th><th className="border border-slate-300 px-4 py-2 text-right text-white font-medium">{t('التاريخ','Date',lang)}</th></tr></thead><tbody>{contract.attachments.map((att:Attachment,i:number)=>(<tr key={att.id} style={{backgroundColor:i%2===0?colors.secondary:'#fff'}}><td className="border border-slate-300 px-4 py-2">{att.title}</td><td className="border border-slate-300 px-4 py-2">{att.attachment_type}</td><td className="border border-slate-300 px-4 py-2">{att.uploaded_at}</td></tr>))}</tbody></table></div>)}
        </div>
      </div>
    </div>
  );
