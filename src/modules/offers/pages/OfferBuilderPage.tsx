...
import { ArrowLeft, MessageSquare, GitBranch, Eye, FileDown } from 'lucide-react';
...
export default function OfferBuilderPage() {
  ...
  async function handleContentChange(sectionId: string, content: string) {
    const next = offer.sections.map(s => s.id === sectionId ? { ...s, content } : s);
    await updateSections(offer.id, next);
  }
  ...
  async function handleTransitionConfirm(entry: WorkflowLogEntry) {
    ...
    const systemNote: OfferNote = {
      id: crypto.randomUUID(), author_name: 'System', author_email: '',
      note_type: 'approval_decision', visibility: 'internal',
      body: [
      ...
      parent_note_id: null, is_system_generated: true, is_pinned: false,
    ...
    if (entry.to_status === 'won') {
      platformBus.emit(PLATFORM_EVENTS.OFFER_WON, {
        offerId: offer.id, offerNumber: offer.offer_number,
        clientName: offer.client_name, totalValue: offer.total_value,
      });
    }
  ...
  return (
    <div className="flex flex-col h-full" dir={dir}>
      ...
      <header className="flex items-center justify-between px-4 py-3 bg-white border-b border-slate-200 flex-shrink-0">
        ...
        <div className="flex items-center gap-2">
          {offer.client_name && (
            <span className="text-sm text-slate-500 hidden md:block">{offer.client_name}</span>
          )}
          {offer.expiry_date && (
            <span className="text-xs text-slate-400 hidden md:block">Expires {offer.expiry_date}</span>
          )}
          {/* Preview button */}
          <button
