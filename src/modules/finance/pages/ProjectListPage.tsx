...
export default function ProjectListPage() {
  ...
  const filteredProjects = projects.filter(p => {
    const matchesSearch = p.name.toLowerCase().includes(searchTerm.toLowerCase()) || getClientName(p.clientId).toLowerCase().includes(searchTerm.toLowerCase());
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">{projects.length} {t('مشروع إجمالاً', 'projects total', lang)} · {activeCount} {t('نشط', 'active', lang)}</p>
        </div>
        <button onClick={() => { setEditingProject(undefined); setIsModalOpen(true); }}
          ...
          <Plus className="-ml-1 mr-2 h-5 w-5" />{t('مشروع جديد', 'New Project', lang)}
        </button>
      </div>
        ...
        <div className="px-6 py-4 border-b border-slate-100 bg-slate-50 flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
          <div className="flex flex-col sm:flex-row gap-3 flex-1">
            <div className="relative rounded-lg shadow-sm max-w-xs flex-1">
              ...
              <input type="text" className="block w-full pl-10 pr-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 bg-white"
          ...
          {filteredProjects.length !== projects.length && (
            <p className="text-xs text-slate-500 whitespace-nowrap">{t('عرض', 'Showing', lang)} {filteredProjects.length} {t('من', 'of', lang)} {projects.length}</p>
          )}
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-slate-100">
              <thead className="bg-slate-50">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase">{t('المشروع', 'Project', lang)}</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase">{t('العميل', 'Client', lang)}</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase">{t('قيمة العقد', 'Contract Value', lang)}</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase">{t('الحالة', 'Status', lang)}</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase">{t('صحة الميزانية', 'Budget Health', lang)}</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase">{t('المراحل', 'Milestones', lang)}</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase">{t('تاريخ الانتهاء', 'End Date', lang)}</th>
              ...
              <tbody className="bg-white divide-y divide-slate-100">
                {filteredProjects.map(project => {
                  ...
                  const isEndDateOverdue = !!project.endDate && isPast(parseISO(project.endDate)) && project.status !== ProjectStatus.Completed && project.status !== ProjectStatus.Cancelled;
                  return (
                    <tr key={project.id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-3">
                          <div className="flex-shrink-0 h-10 w-10 bg-indigo-50 rounded-xl flex items-center justify-center text-indigo-600 font-bold text-sm">{project.name.charAt(0).toUpperCase()}</div>
                          <div>
                            <div className="text-sm font-semibold text-slate-900">{project.name}</div>
                            <div className="text-xs text-slate-500">{t('بدأ في', 'Started', lang)} {project.startDate ? format(new Date(project.startDate), 'MMM d, yyyy') : '—'}</div>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2.5 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusColor(project.status)}`}>{tEnum(project.status, lang)}</span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="w-full max-w-[140px]">
                          ...
                          <div className="w-full bg-slate-200 rounded-full h-2">
                            <div className={`h-2 rounded-full transition-all ${budgetPct > 100 ? 'bg-rose-500' : budgetPct > 80 ? 'bg-amber-500' : 'bg-emerald-500'}`} style={{ width: `${Math.min(budgetPct, 100)}%` }} />
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        <span className={`font-semibold ${completedMs === project.milestones.length && project.milestones.length > 0 ? 'text-emerald-600' : 'text-slate-700'}`}>{completedMs}</span>
                        <span className="text-slate-400"> / {project.milestones.length}</span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        {project.endDate ? (
                          <span className={`inline-flex items-center gap-1 ${isEndDateOverdue ? 'text-rose-600 font-medium' : 'text-slate-500'}`}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <Link to={`/finance/projects/${project.id}`} className="text-indigo-600 hover:text-indigo-800 transition-colors"><ChevronRight className="h-5 w-5" /></Link>
                      </td>
                    </tr>
                  );
                })}
              ...
              {filteredProjects.length > 0 && (
                <tfoot className="bg-slate-50 border-t-2 border-slate-200">
                  <tr><td className="px-6 py-3 text-xs font-medium text-slate-500" colSpan={8}>{t('عرض', 'Showing', lang)} {filteredProjects.length} {t('مشروع', filteredProjects.length === 1 ? 'project' : 'projects', lang)} · {activeCount} {t('نشط', 'active', lang)}</td></tr>
                </tfoot>
              )}
            </table>
          </div>
      ...
      {isModalOpen && (
        <ProjectForm initialData={editingProject} onSave={editingProject ? handleUpdate : handleCreate}
