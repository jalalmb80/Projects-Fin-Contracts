...
export default function ProjectListPage() {
  const { projects, counterparties, addProject, updateProject, loading, settings } = useApp();
  const { lang } = useLang();
  ...
  const getClientName = (clientId: string) => {
    return counterparties.find(c => c.id === clientId)?.name || t('عميل غير معروف', 'Unknown Client', lang);
  };
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{t('المشروع', 'Project', lang)}</th>
                  <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{t('العميل', 'Client', lang)}</th>
                  <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{t('العقد', 'Contract', lang)}</th>
                  <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{t('الحالة', 'Status', lang)}</th>
                  <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{t('صحة الميزانية', 'Budget Health', lang)}</th>
                  <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{t('المراحل', 'Milestones', lang)}</th>
                  <th scope="col" className="relative px-6 py-3"><span className="sr-only">{t('عرض', 'View', lang)}</span></th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredProjects.map((project) => {
                  const totalBudget = project.wbs.reduce((sum, w) => sum + w.budget, 0);
                  const totalSpent = project.wbs.reduce((sum, w) => sum + w.spent, 0);
                  const budgetPercent = totalBudget > 0 ? (totalSpent / totalBudget) * 100 : 0;
                  const completedMilestones = project.milestones.filter(m => m.status === 'Completed' || m.status === 'Invoiced').length;
                      ...
                      <td className="px-6 py-4 whitespace-nowrap align-middle">
                        <div className="w-full max-w-[140px]">
                          ...
                          <div className="w-full bg-gray-200 rounded-full h-2">
                            <div 
                              className={`h-2 rounded-full ${budgetPercent > 100 ? 'bg-red-500' : budgetPercent > 80 ? 'bg-yellow-500' : 'bg-green-500'}`} 
