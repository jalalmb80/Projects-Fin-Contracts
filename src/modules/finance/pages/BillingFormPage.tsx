  ...
  useEffect(() => {
    const off = platformBus.on(PLATFORM_EVENTS.CONTRACT_SIGNED, (payload) => {
      setFormData(prev => ({
        ...
        notes: `Auto-generated from contract: ${payload.contractTitle || payload.contractId}`,
        lines: [{
          id: crypto.randomUUID(),
          description: payload.contractTitle || `Contract ${payload.contractId}`,
          quantity: 1,
          unitPrice: payload.amount || 0,
          taxCode: TaxProfile.Standard,
          taxAmount: (payload.amount || 0) * 0.15,
          subtotal: payload.amount || 0,
          total: (payload.amount || 0) * 1.15,
        }],
      }));
    });
    return off;
