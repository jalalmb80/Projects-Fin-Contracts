...
import { 
  ChevronLeft, CreditCard, CheckCircle, AlertCircle, FileText, Edit2, Send, XCircle
} from 'lucide-react';
...
export default function BillingDetailPage() {
  ...
  const { 
    billingDocuments, counterparties, legalEntities, payments,
    issueDocument, submitForApproval, approveDocument, markAsSent,
    voidDocument, returnToDraft, recordPayment, allocatePayment, loading
  } = useApp();
  ...
  if (!document) {
    return (
      <div className="text-center py-12">
        ...
        <Link to="/finance/billing" className="mt-4 text-indigo-600 hover:text-indigo-500">Back to Billing</Link>
      </div>
    );
  }
  ...
  const relatedPayments = payments.flatMap(p =>
    p.allocations.filter(a => a.invoiceId === document.id).map(a => ({
      paymentId: p.id, date: p.date, method: p.method, reference: p.reference, amount: a.amount, currency: p.currency
    }))
  ...
  const handlePaymentSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (paymentAmount <= 0 || paymentAmount > document.balance) { addToast('error', 'Invalid payment amount'); return; }
    try {
      ...
      <div className="bg-white shadow rounded-lg p-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="flex items-start space-x-4">
            <Link to="/finance/billing" className="mt-1 text-gray-400 hover:text-gray-500"><ChevronLeft className="h-6 w-6" /></Link>
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">Created on {new Date(document.date).toLocaleDateString()} • Due {new Date(document.dueDate).toLocaleDateString()}</p>
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            {document.status === DocumentStatus.PendingApproval && (
              <>
                <button onClick={() => approveDocument(document.id)} className="bg-green-600 text-white text-sm px-3 py-2 rounded hover:bg-green-700 flex items-center"><CheckCircle className="h-4 w-4 mr-1" /> Approve</button>
            ...
            {(document.status === DocumentStatus.Issued || document.status === DocumentStatus.Sent || document.status === DocumentStatus.PartiallyPaid || document.status === DocumentStatus.Overdue) && (
              <>
                ...
                {document.status === DocumentStatus.Issued && (
                  <button onClick={() => markAsSent(document.id)} className="bg-blue-600 text-white text-sm px-3 py-2 rounded hover:bg-blue-700 flex items-center"><Send className="h-4 w-4 mr-1" /> Mark as Sent</button>
                )}
                <button onClick={() => voidDocument(document.id)} className="bg-red-50 text-red-600 text-sm px-3 py-2 rounded hover:bg-red-100 flex items-center"><XCircle className="h-4 w-4 mr-1" /> Void</button>
              </>
            )}
          </div>
        </div>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {document.lines.map(line => (
                    <tr key={line.id}>
                      ...
                      <td className="px-6 py-4 text-sm text-right text-gray-500">{line.quantity}</td>
                      <td className="px-6 py-4 text-sm text-right text-gray-500">{formatCurrency(line.unitPrice)}</td>
                      <td className="px-6 py-4 text-sm text-right text-gray-500">{formatCurrency(line.taxAmount)}</td>
                      <td className="px-6 py-4 text-sm text-right font-medium text-gray-900">{formatCurrency(line.total)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {relatedPayments.map((p, i) => (
                    <tr key={`${p.paymentId}-${i}`}>
                      ...
                      <td className="px-6 py-4 text-sm text-right font-medium text-gray-900">{formatCurrency(p.amount)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="space-y-6">
          <div className="bg-white shadow rounded-lg p-6">
            ...
            <div className="flex items-center mb-4">
              <div className="h-10 w-10 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 font-bold text-lg">{document.counterpartyName.charAt(0)}</div>
              <div className="ml-3">
            ...
            {document.direction === DocumentDirection.IC && fromEntity && toEntity && (
              <div className="mt-4 pt-4 border-t border-gray-100">
                ...
                <div className="flex items-center text-sm">
                  <span className="font-medium">{fromEntity.name}</span><span className="mx-2 text-gray-400">→</span><span className="font-medium">{toEntity.name}</span>
                </div>
              </div>
            )}
            ...
            <form onSubmit={handlePaymentSubmit} className="space-y-4">
              ...
              <div className="mt-5 flex space-x-3">
                <button type="button" onClick={() => setIsPaymentModalOpen(false)}
                  className="w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 sm:text-sm">Cancel</button>
                <button type="submit"
                  className="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-indigo-600 text-base font-medium text-white hover:bg-indigo-700 sm:text-sm">Record Payment</button>
              </div>
            </form>
          </div>
        </div>
