  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex flex-wrap gap-2">
            {document.status === DocumentStatus.Draft && (
              <>
                ...
                <button onClick={() => navigate(`/finance/billing/new?id=${document.id}`)} className="text-sm px-3 py-2 border rounded hover:bg-gray-50 flex items-center"><Edit2 className="h-4 w-4 mr-1" /> Edit</button>
              </>
            )}
            ...
            {document.status === DocumentStatus.Approved && (
              <button onClick={() => issueDocument(document.id)} className="bg-indigo-600 text-white text-sm px-3 py-2 rounded hover:bg-indigo-700 flex items-center"><FileText className="h-4 w-4 mr-1" /> Issue Document</button>
            )}
            {(document.status === DocumentStatus.Issued || document.status === DocumentStatus.Sent || document.status === DocumentStatus.PartiallyPaid || document.status === DocumentStatus.Overdue) && (
              <>
                <button onClick={() => { setPaymentAmount(document.balance); setIsPaymentModalOpen(true); }} className="bg-green-600 text-white text-sm px-3 py-2 rounded hover:bg-green-700 flex items-center"><CreditCard className="h-4 w-4 mr-1" /> Record Payment</button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          <div className="bg-white shadow rounded-lg p-6">
            <h3 className="text-sm font-medium text-gray-500 uppercase tracking-wider mb-4">{document.direction === DocumentDirection.AR ? 'Bill To' : 'Bill From'}</h3>
            ...
            {document.direction === DocumentDirection.IC && fromEntity && toEntity && (
              <div className="mt-4 pt-4 border-t border-gray-100">
                ...
                <div className="flex items-center text-sm"><span className="font-medium">{fromEntity.name}</span><span className="mx-2 text-gray-400">→</span><span className="font-medium">{toEntity.name}</span></div>
              </div>
            )}
            ...
            <form onSubmit={handlePaymentSubmit} className="space-y-4">
              ...
              <div className="mt-5 sm:mt-6 flex space-x-3">
                <button type="button" onClick={() => setIsPaymentModalOpen(false)}
                  className="w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none sm:text-sm">Cancel</button>
                <button type="submit"
                  className="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-indigo-600 text-base font-medium text-white hover:bg-indigo-700 focus:outline-none sm:text-sm">Record Payment</button>
              </div>
            </form>
          </div>
        </div>
