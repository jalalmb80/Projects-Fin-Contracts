    ...
    <div className="space-y-6">
        ...
        {activeTab === 'overview' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-3 grid grid-cols-1 sm:grid-cols-3 gap-5">
              <div className="bg-white overflow-hidden shadow rounded-lg p-5"><div className="flex items-center"><div className="flex-shrink-0 bg-indigo-100 rounded-md p-3"><DollarSign className="h-6 w-6 text-indigo-600" /></div><div className="ml-5 w-0 flex-1"><dt className="text-sm font-medium text-gray-500 truncate">{t('نسبة استهلاك الميزانية', 'Budget Consumed', lang)}</dt><dd className="text-2xl font-semibold text-gray-900">{Math.round(budgetProgress)}%</dd></div></div><div className="mt-4 w-full bg-gray-200 rounded-full h-2"><div className={`h-2 rounded-full ${budgetProgress > 100 ? 'bg-red-500' : budgetProgress > 80 ? 'bg-yellow-500' : 'bg-indigo-600'}`} style={{ width: `${Math.min(budgetProgress, 100)}%` }} /></div></div>
              <div className="bg-white overflow-hidden shadow rounded-lg p-5"><div className="flex items-center"><div className="flex-shrink-0 bg-green-100 rounded-md p-3"><CheckCircle className="h-6 w-6 text-green-600" /></div><div className="ml-5 w-0 flex-1"><dt className="text-sm font-medium text-gray-500 truncate">{t('المراحل المكتملة', 'Milestones Completed', lang)}</dt><dd className="text-2xl font-semibold text-gray-900">{completedMilestones} / {totalMilestones}</dd></div></div><div className="mt-4 w-full bg-gray-200 rounded-full h-2"><div className="bg-green-600 h-2 rounded-full" style={{ width: `${Math.min(milestoneProgress, 100)}%` }} /></div></div>
              <div className="bg-white overflow-hidden shadow rounded-lg p-5"><div className="flex items-center"><div className="flex-shrink-0 bg-blue-100 rounded-md p-3"><FileText className="h-6 w-6 text-blue-600" /></div><div className="ml-5 w-0 flex-1"><dt className="text-sm font-medium text-gray-500 truncate">{t('الفواتير الصادرة', 'Invoices Issued', lang)}</dt><dd className="text-2xl font-semibold text-gray-900">{projectDocs.filter(d => d.direction === DocumentDirection.AR && d.status !== DocumentStatus.Draft).length}</dd></div></div></div>
            </div>
          </div>
        )}
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {project.milestones.map(milestone => (
                    <tr key={milestone.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-center"><span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getMilestoneStatusColor(milestone.status)}`}>{tEnum(milestone.status, lang)}</span></td>
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex justify-end items-center space-x-3">
                          ...
                          {milestone.status === MilestoneStatus.Invoiced && milestone.linkedInvoiceId && (
                            <Link to={`/finance/billing/${milestone.linkedInvoiceId}`} className="text-indigo-600 hover:text-indigo-900"><FileText className="h-4 w-4" /></Link>
                          )}
                  ...
                  {projectBudgetCategories.map(item => {
                    ...
                    return (
                      <tr key={item.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap align-middle"><div className="w-full bg-gray-200 rounded-full h-2.5"><div className={`h-2.5 rounded-full ${percent >= 100 ? 'bg-red-600' : percent >= 80 ? 'bg-yellow-500' : 'bg-green-600'}`} style={{ width: `${Math.min(percent, 100)}%` }} /></div><div className="text-xs text-center mt-1 text-gray-500">{Math.round(percent)}%</div></td>
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium"><div className="flex justify-end space-x-2"><button onClick={() => handleOpenBudgetModal(item)} className="text-indigo-600 hover:text-indigo-900"><Edit2 className="h-4 w-4" /></button><button onClick={() => { setBudgetCategoryToDelete(item.id); setIsDeleteBudgetDialogOpen(true); }} className="text-red-600 hover:text-red-900"><Trash2 className="h-4 w-4" /></button></div></td>
                      </tr>
                    );
                  })}
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {projectDocs.map(doc => (
                    <tr key={doc.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap"><span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${doc.status === DocumentStatus.Paid ? 'bg-green-100 text-green-800' : doc.status === DocumentStatus.Overdue ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-800'}`}>{tEnum(doc.status, lang)}</span></td>
      ...
      <Modal isOpen={isMilestoneModalOpen} onClose={() => setIsMilestoneModalOpen(false)}
        ...
        <div className="space-y-4">
          <div><label className="block text-sm font-medium text-gray-700">{t('الاسم', 'Name', lang)}</label><input type="text" required value={milestoneForm.name || ''} onChange={e => setMilestoneForm({ ...milestoneForm, name: e.target.value })} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" /></div>
          <div><label className="block text-sm font-medium text-gray-700">{t('الوصف', 'Description', lang)}</label><textarea rows={3} value={milestoneForm.description || ''} onChange={e => setMilestoneForm({ ...milestoneForm, description: e.target.value })} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" /></div>
          <div><label className="block text-sm font-medium text-gray-700">{t('تاريخ الاستحقاق', 'Due Date', lang)}</label><input type="date" required value={milestoneForm.dueDate || ''} onChange={e => setMilestoneForm({ ...milestoneForm, dueDate: e.target.value })} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" /></div>
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-sm font-medium text-gray-700">{t('% من العقد', '% of Contract', lang)}</label><div className="mt-1 relative rounded-md shadow-sm"><input type="number" min="0" max="100" value={milestoneForm.percentOfContract || 0} onChange={e => handleMilestonePercentChange(parseFloat(e.target.value) || 0)} className="focus:ring-indigo-500 focus:border-indigo-500 block w-full pr-8 sm:text-sm border-gray-300 rounded-md" /><div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none"><span className="text-gray-500 sm:text-sm">%</span></div></div></div>
            <div><label className="block text-sm font-medium text-gray-700">{t('المبلغ', 'Amount', lang)}</label><div className="mt-1 relative rounded-md shadow-sm"><div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><span className="text-gray-500 sm:text-sm">{project.baseCurrency}</span></div><input type="number" value={milestoneForm.amount || 0} onChange={e => setMilestoneForm({ ...milestoneForm, amount: parseFloat(e.target.value) || 0 })} className="focus:ring-indigo-500 focus:border-indigo-500 block w-full pl-10 sm:text-sm border-gray-300 rounded-md" /></div></div>
          </div>
          <div><label className="block text-sm font-medium text-gray-700">{t('الحالة', 'Status', lang)}</label><select value={milestoneForm.status || MilestoneStatus.Pending} onChange={e => setMilestoneForm({ ...milestoneForm, status: e.target.value as MilestoneStatus })} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">{Object.values(MilestoneStatus).map(s => <option key={s} value={s}>{tEnum(s, lang)}</option>)}</select></div>
          <div className="pt-4"><button onClick={handleSaveMilestone} className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none">{t('حفظ المرحلة', 'Save Milestone', lang)}</button></div>
        </div>
      </Modal>
      ...
      <Modal isOpen={isBudgetModalOpen} onClose={() => setIsBudgetModalOpen(false)}
        ...
        <div className="space-y-4">
          <div><label className="block text-sm font-medium text-gray-700">{t('اسم الفئة', 'Category Name', lang)}</label><input type="text" required value={budgetForm.name || ''} onChange={e => setBudgetForm({ ...budgetForm, name: e.target.value })} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" /></div>
          <div><label className="block text-sm font-medium text-gray-700">{t('المبلغ المخطط', 'Planned Amount', lang)}</label><div className="mt-1 relative rounded-md shadow-sm"><div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><span className="text-gray-500 sm:text-sm">{project.baseCurrency}</span></div><input type="number" value={budgetForm.planned || 0} onChange={e => setBudgetForm({ ...budgetForm, planned: parseFloat(e.target.value) || 0 })} className="focus:ring-indigo-500 focus:border-indigo-500 block w-full pl-10 sm:text-sm border-gray-300 rounded-md" /></div></div>
          <div><label className="block text-sm font-medium text-gray-700">{t('المبلغ الفعلي', 'Actual Amount', lang)}</label><div className="mt-1 relative rounded-md shadow-sm"><div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><span className="text-gray-500 sm:text-sm">{project.baseCurrency}</span></div><input type="number" min="0" value={budgetForm.actual || 0} onChange={e => setBudgetForm({ ...budgetForm, actual: parseFloat(e.target.value) || 0 })} className="focus:ring-indigo-500 focus:border-indigo-500 block w-full pl-10 sm:text-sm border-gray-300 rounded-md" /></div></div>
          <div><label className="block text-sm font-medium text-gray-700">{t('ملاحظات (اختياري)', 'Notes (Optional)', lang)}</label><textarea rows={3} value={budgetForm.notes || ''} onChange={e => setBudgetForm({ ...budgetForm, notes: e.target.value })} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" /></div>
          <div className="pt-4"><button onClick={handleSaveBudgetCategory} className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none">{t('حفظ الفئة', 'Save Category', lang)}</button></div>
        </div>
      </Modal>
