...
export default function ProjectDetailPage() {
  ...
  const handleSaveMilestone = async () => {
    ...
    if (currentTotalPercent + (milestoneForm.percentOfContract || 0) > 100) {
      addToast('error', t('لا يمكن أن يتجاوز مجموع نسب المراحل 100%', 'Total milestone percentage cannot exceed 100%', lang));
      return;
    }
    try {
      if (editingMilestone) {
  ...
  const handleCompleteMilestone = async (milestoneId: string) => {
    ...
    try {
      ...
      console.error(error);
      addToast('error', t('فشل اعتبار المرحلة مكتملة', 'Failed to complete milestone', lang));
    } finally {
  ...
  const handleOpenBudgetModal = (category?: BudgetCategory) => {
    if (category) {
      ...
      setBudgetForm({ name: '', planned: 0, actual: 0, notes: '' });
    }
    ...
    <div className="space-y-6">
          ...
          <div className="flex items-start space-x-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">
                ...
                <span className="font-medium text-gray-900">{client?.name || t('غير معروف', 'Unknown', lang)}</span>
                {' · '}
                {t('نوع العقد', 'Contract', lang)}:{' '}
                <span className="font-medium text-gray-900">{tEnum(project.contractType, lang)}</span>
              </p>
            </div>
          </div>
      ...
      <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="mt-4 w-full bg-gray-200 rounded-full h-2">
                  <div
                    className={`h-2 rounded-full ${
                      ...
                      budgetProgress > 80  ? 'bg-yellow-500' : 'bg-primary-600'
                    }`}
            ...
            <div className="lg:col-span-3 bg-white shadow rounded-lg overflow-hidden">
              <div className="px-6 py-5 border-b border-gray-200">
                <h3 className="text-lg font-medium leading-6 text-gray-900">{t('تفصيل الميزانية (WBS)', 'Budget Breakdown (WBS)', lang)}</h3>
              </div>
                    ...
                    {projectBudgetCategories.length === 0 && (
                      <tr>
                        <td colSpan={5} className="px-6 py-4 text-center text-sm text-gray-500">
                          {t('لا توجد فئات ميزانية. اذهب إلى تبويب الميزانية لإضافة بنود.', 'No budget categories defined. Go to the Budget tab to add items.', lang)}
                        </td>
                      </tr>
                    )}
                  </tbody>
                        ...
                        <div className="flex justify-end items-center space-x-3">
                          ...
                          {milestone.status === MilestoneStatus.Invoiced && milestone.linkedInvoiceId && (
                            <Link
                              ...
                              title={t('عرض الفاتورة', 'View Invoice', lang)}
                            >
                          ...
                          <button
                            onClick={() => { setMilestoneToDelete(milestone.id); setIsDeleteMilestoneDialogOpen(true); }}
                  ...
                  {project.milestones.length === 0 && (
                    <tr>
                      <td colSpan={6} className="px-6 py-4 text-center text-sm text-gray-500">
                        {t('لا توجد مراحل.', 'No milestones found.', lang)}
                      </td>
                    </tr>
                  )}
                </tbody>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                      ...
                      <tr key={item.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                          <div className="flex justify-end space-x-2">
                            <button onClick={() => handleOpenBudgetModal(item)} className="text-indigo-600 hover:text-indigo-900">
                              <Edit2 className="h-4 w-4" />
                            </button>
                            <button
                              onClick={() => { setBudgetCategoryToDelete(item.id); setIsDeleteBudgetDialogOpen(true); }}
                  ...
                  {projectBudgetCategories.length === 0 && (
                    <tr>
                      <td colSpan={6} className="px-6 py-4 text-center text-sm text-gray-500">
                        {t('لا توجد فئات ميزانية.', 'No budget categories found.', lang)}
                      </td>
                    </tr>
                  )}
                </tbody>
        ...
        {activeTab === 'documents' && (
          <div className="bg-white shadow rounded-lg overflow-hidden">
            <div className="px-6 py-5 border-b border-gray-200">
              <h3 className="text-lg font-medium leading-6 text-gray-900">{t('المستندات ذات الصلة', 'Related Documents', lang)}</h3>
            </div>
            {projectDocs.length === 0 ? (
              <div className="p-6 text-center text-gray-500">
                {t('لا توجد مستندات لهذا المشروع.', 'No documents found for this project.', lang)}
              </div>
            ) : (
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  ...
                  {projectDocs.map(doc => (
                    <tr key={doc.id}>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-indigo-600">
                        {doc.documentNumber || t('مسودة', 'Draft', lang)}
                      </td>
      ...
      >
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">{t('الوصف', 'Description', lang)}</label>
            <textarea
              rows={3}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">{t('الحالة', 'Status', lang)}</label>
            <select
            ...
            >
              {Object.values(MilestoneStatus).map(status => (
                <option key={status} value={status}>{tEnum(status, lang)}</option>
              ))}
            </select>
          </div>
          <div className="pt-4">
      ...
      <ConfirmDialog
        ...
        title={t('حذف المرحلة', 'Delete Milestone', lang)}
        message={t('هل أنت متأكد من حذف هذه المرحلة؟', 'Are you sure you want to delete this milestone?', lang)}
      />
      ...
      >
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">{t('ملاحظات (اختياري)', 'Notes (Optional)', lang)}</label>
            <textarea
              rows={3}
      ...
      <ConfirmDialog
        ...
        title={t('حذف فئة الميزانية', 'Delete Budget Category', lang)}
        message={t('هل أنت متأكد من حذف هذه الفئة؟', 'Are you sure you want to delete this budget category?', lang)}
      />
    </div>
  );
}
