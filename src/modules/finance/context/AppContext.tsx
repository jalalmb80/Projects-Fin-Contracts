...
export const AppProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    const snap = (col: keyof AppState['loading'], q: Query<any>) =>
      onSnapshot(
        q,
  ...
  const addProject = async (project: Omit<Project, 'id' | 'createdAt' | 'updatedAt'>) => {
    try { const id = generateId(); await setDoc(doc(db, 'projects', id), { ...project, id, createdAt: now(), updatedAt: now() }); addToast('success', 'Project created successfully'); }
    catch (error) { addToast('error', 'Failed to create project'); throw error; }
  };
  const updateProject = async (id: string, data: Partial<Project>) => {
    try { await updateDoc(doc(db, 'projects', id), { ...data, updatedAt: now() }); addToast('success', 'Project updated successfully'); }
    catch (error) { addToast('error', 'Failed to update project'); throw error; }
  };
  const deleteProject = async (id: string) => {
    try { await deleteDoc(doc(db, 'projects', id)); addToast('success', 'Project deleted successfully'); }
  ...
  const updateMilestone = async (projectId: string, milestoneId: string, data: Partial<Milestone>) => {
    try {
      const project = state.projects.find(p => p.id === projectId);
      if (!project) throw new Error('Project not found');
      await updateDoc(doc(db, 'projects', projectId), { milestones: project.milestones.map(m => m.id === milestoneId ? { ...m, ...data } : m), updatedAt: now() });
  ...
  const deleteMilestone = async (projectId: string, milestoneId: string) => {
    try {
      const project = state.projects.find(p => p.id === projectId);
      if (!project) throw new Error('Project not found');
      await updateDoc(doc(db, 'projects', projectId), { milestones: project.milestones.filter(m => m.id !== milestoneId), updatedAt: now() });
  ...
  const addBillingDocument = async (docData: Omit<BillingDocument, 'id' | 'createdAt' | 'updatedAt'>) => {
    try { const id = generateId(); await setDoc(doc(db, 'billingDocuments', id), { ...docData, id, createdAt: now(), updatedAt: now() }); addToast('success', 'Document created'); }
  ...
  const updateBillingDocument = async (id: string, data: Partial<BillingDocument>) => {
    try { await updateDoc(doc(db, 'billingDocuments', id), { ...data, updatedAt: now() }); addToast('success', 'Document updated'); }
  ...
  const voidDocument = async (id: string) => {
    try {
      const linkedPaymentIds = state.payments.filter(p => p.allocations.some(a => a.invoiceId === id)).map(p => p.id);
      await runTransaction(db, async (transaction) => {
        ...
        for (const paySnap of paymentSnaps) {
          ...
          transaction.update(paySnap.ref, { allocations: payment.allocations.filter(a => a.invoiceId !== id), unallocatedAmount: (payment.unallocatedAmount ?? 0) + reversed });
        }
      });
  ...
  const recordPayment = async (payment: Omit<Payment, 'id' | 'createdAt'>): Promise<string> => {
    const id = generateId();
    try { await setDoc(doc(db, 'payments', id), { ...payment, id, createdAt: now() }); addToast('success', 'Payment recorded'); return id; }
  ...
  const addSubscription = async (sub: Omit<Subscription, 'id' | 'createdAt' | 'updatedAt'>) => {
    try { const id = generateId(); await setDoc(doc(db, 'subscriptions', id), { ...sub, id, createdAt: now(), updatedAt: now() }); addToast('success', 'Subscription created'); }
  ...
  const addCounterparty = async (cp: Omit<Counterparty, 'id' | 'createdAt'>) => {
    try { const id = generateId(); await setDoc(doc(db, 'counterparties', id), { ...cp, id, createdAt: now() }); addToast('success', 'Counterparty added'); }
