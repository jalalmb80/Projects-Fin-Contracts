...
export default function Layout() {
  ...
  const overdueCount = billingDocuments.filter(d => d.direction === DocumentDirection.AR && d.status === DocumentStatus.Overdue).length;
  const toggleCurrency = () => setDisplayCurrency(displayCurrency === Currency.SAR ? Currency.USD : Currency.SAR);
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex" dir={isRTL ? 'rtl' : 'ltr'}>
      <div className={cn('lg:hidden fixed top-4 z-50', isRTL ? 'right-4' : 'left-4')}>
        <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="p-2 rounded-md bg-slate-900 shadow-md text-slate-300 hover:text-white">
      ...
      <div className={cn(
        ...
        <div className="flex items-center justify-between h-16 px-4 border-b border-slate-800">
          ...
          <button onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)} className="hidden lg:block p-1 rounded-md hover:bg-slate-800 text-slate-400 hover:text-white">
            ...
            {navigation.map(item => {
              ...
              return (
                <Link key={item.name_en} to={item.href}
                  className={cn('group flex items-center px-2 py-2 text-sm font-medium rounded-lg transition-colors', isActive ? 'bg-indigo-600 text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white', isSidebarCollapsed ? 'justify-center' : '')}
                  title={isSidebarCollapsed ? itemName : undefined} onClick={() => setIsMobileMenuOpen(false)}>
                  <item.icon className={cn('flex-shrink-0 h-5 w-5', isActive ? 'text-white' : 'text-slate-400 group-hover:text-white', !isSidebarCollapsed && (isRTL ? 'ml-3' : 'mr-3'))} />
        ...
        <div className="border-t border-slate-800 p-4 space-y-3">
          {!isSidebarCollapsed ? (
            <div className="flex items-center">
              <div className="flex-shrink-0">{user?.photoURL ? <img className="h-8 w-8 rounded-full" src={user.photoURL} alt="" /> : <div className="h-8 w-8 rounded-full bg-slate-700 flex items-center justify-center text-indigo-400 font-bold">{user?.email?.charAt(0).toUpperCase()}</div>}</div>
              <div className={cn('overflow-hidden', isRTL ? 'mr-3' : 'ml-3')}><p className="text-sm font-medium text-slate-300 truncate">{user?.displayName || user?.email}</p></div>
            </div>
          ) : (
            <div className="flex justify-center"><div className="h-8 w-8 rounded-full bg-slate-700 flex items-center justify-center text-indigo-400 font-bold">{user?.email?.charAt(0).toUpperCase()}</div></div>
          )}
          <button onClick={() => setLang(lang === 'ar' ? 'en' : 'ar')}
            className={cn('w-full flex items-center px-2 py-2 text-sm font-medium text-slate-300 rounded-lg bg-slate-800 hover:bg-slate-700 transition-colors', isSidebarCollapsed ? 'justify-center' : 'space-x-2 space-x-reverse')}
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="bg-white border-b border-slate-200 shadow-sm h-16 flex items-center justify-between px-4 sm:px-6 lg:px-8">
          <h2 className="text-lg font-medium text-slate-900">
            {(() => { const m = navigation.find(n => location.pathname.startsWith(n.href)); return m ? t(m.name_ar, m.name_en, lang) : t('لوحة التحكم', 'Dashboard', lang); })()}
          </h2>
          <div className="flex items-center space-x-4 space-x-reverse">
            ...
            <button onClick={toggleCurrency} className="px-3 py-1 text-sm font-medium text-slate-700 bg-slate-100 rounded-md hover:bg-slate-200">{displayCurrency}</button>
            ...
            <div className="relative">
              ...
              {overdueCount > 0 && <span className="absolute -top-1 -right-1 h-4 w-4 bg-red-500 rounded-full flex items-center justify-center text-[10px] text-white font-bold">{overdueCount}</span>}
            </div>
          </div>
        </header>
